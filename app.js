...
function positionHeaderUserMenu() {
    ...
    const trigger = window.matchMedia('(max-width: 768px)').matches && mobileTrigger ? mobileTrigger : headerTrigger;
...
function renderDuplicatePoBadge(order, duplicatePoSet) {
    ...
    return `<div class="duplicate-po-badge" title="Duplicate PO in this list"><i class="fa-solid fa-copy"></i> Duplicate</div>`;
}
...
async function renderDetailPhotoPreviewStrip(order) {
    ...
    if (!container) return;
    const allPhotos = getRecordPhotoFiles(order);
    const photos = allPhotos.slice(0, 12);
    if (!photos.length) {
        container.innerHTML = `<div class="record-photo-empty"><i class="fa-regular fa-images"></i><span>No photos yet</span></div>`;
        return;
    }
    const thumbs = await Promise.all(photos.map(async file => {
        ...
        try {
            ...
            return `<button type="button" class="record-preview-thumb-btn photo-preview-trigger" data-preview-folder="${escapeHtml(file.folder)}" data-preview-filename="${escapeHtml(file.filename)}" onclick="openRecordPhotoPreview('${escapeJsString(order.id)}', '${escapeJsString(file.filename)}')" title="Open photo">
    ...
    container.innerHTML = `<div class="record-photo-strip-head"><span>${allPhotos.length} photo${allPhotos.length === 1 ? '' : 's'}</span></div><div class="record-photo-strip-scroller">${thumbs.join('')}${remaining > 0 ? `<button type="button" class="record-preview-more" onclick="openRecordPhotoPreview('${escapeJsString(order.id)}', '${escapeJsString(photos[0]?.filename || '')}')">+${remaining}</button>` : ''}</div>`;
}
...
function renderStockTable() {
    ...
    if (items.length === 0) {
        ...
        if (stockEmptyText) stockEmptyText.textContent = stockFiltered ? 'Clear search or choose a different stock filter.' : 'Add stock items from Admin Settings to start tracking inventory.';
