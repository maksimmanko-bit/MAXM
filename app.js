...
function isReceivingOrder(order) {
    return !isOutboundOrder(order) && !isPickupOrder(order);
}
...
function updateDashboardChrome() {
    ...
    const isReceiving = !isOutbound && !isPickup;
    ...
    document.getElementById('carrierFilterWrapper')?.classList.toggle('hidden', isPickup);
    
    const tableTitle = document.getElementById('shipmentTableTitle');
    if (tableTitle) tableTitle.textContent = isPickup ? 'Customer Pick Up Logs' : (isOutbound ? 'Shipped Out Logs' : 'Receiving Logs');
    
    const searchInput = document.getElementById('searchInput');
    if (searchInput) {
        searchInput.placeholder = isPickup
    ...
    if (emptyText) {
        emptyText.textContent = isPickup
...
function renderDashboard() {
    ...
    const filteredOrders = orders.filter(order => {
        ...
        // Status filter
        if (!isOutboundTab && !isPickupTab && filterStatus !== 'all' && order.status !== filterStatus) return false;
        
        // Carrier filter
        if (!isPickupTab && filterCarrier !== 'all') {
        ...
        // Search text
        if (searchVal) {
            const searchableFields = isPickupTab
            ...
            const matchesSearch = searchableFields.some(value => value && value.toLowerCase().includes(searchVal));
    ...
    if (filteredOrders.length === 0) {
        ...
        document.getElementById('recordCount').textContent = isPickupTab ? 'Showing 0 pick up records' : (isOutboundTab ? 'Showing 0 shipped out records' : 'Showing 0 orders');
        return;
    } else {
        emptyState.classList.add('hidden');
        document.getElementById('recordCount').textContent = isPickupTab
    ...
    filteredOrders.forEach(order => {
        ...
        if (isPickupTab) {
            ...
            tr.innerHTML = `
                ...
                <td class="po-col">${escapeHtml(order.job_number)}</td>
...
function updateEtaNotificationBadge() {
    const badge = document.getElementById('etaNotificationBadge');
    if (!badge) return;
    const attentionCount = orders.filter(isReceivingOrder).filter(order => {
        const state = getEtaState(order);
        return state && state.difference <= 0;
    }).length;
...
async function deleteOrder(orderId) {
    ...
    if (isOutboundOrder(order) || isPickupOrder(order)) {
        alert('Completed handoff records can only be deleted from Admin Settings.');
        return;
    }
...
function renderAdminShipmentsList() {
    ...
    filtered.forEach(order => {
        ...
        const reference = (outbound || pickup) ? (order.job_number || '-') : order.po_number;
        const party = pickup ? (order.customer_name || '-') : (outbound ? order.customer : order.supplier);
        const status = pickup ? 'Picked Up' : (outbound ? 'Shipped Out' : order.status);
        const statusClass = (outbound || pickup) ? 'in-transit' : order.status.toLowerCase().replace(' ', '-');
        const typeLabel = pickup ? 'PICKUP' : (outbound ? 'OUT' : 'IN');
...
async function adminDeleteOrder(orderId) {
    ...
    if (!outbound && !pickup && order.status === 'Received') {
