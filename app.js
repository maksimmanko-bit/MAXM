...
let tableDensity = localStorage.getItem('max-logistics-table-density') || 'comfortable';
...
function getRecordFiles(order) {
    ...
    return [
        ...(order.packing_slip_filename ? [{ folder: 'scans', filename: order.packing_slip_filename, downloadName: `Packing_Slip_${order.packing_slip_filename}` }] : []),
...
function renderTableHeader() {
    ...
    tableHead.innerHTML = `<tr>${getSavedColumnOrder().map(key => {
        ...
        return `<th data-column="${key}" ${sortKey ? `class="sortable" data-sort="${sortKey}"` : ''}>${label}</th>`;
    }).join('')}</tr>`;
    setupSortableHeaders();
}

function recordRowState(order) {
    if (order.has_issue || ['Denied', 'Inspected - damage found'].includes(order.status)) return 'row-problem';
    if (isReceivingOrder(order)) {
        const eta = getEtaState(order);
        if (eta && eta.difference <= 0 && order.status === 'Ordered') return 'row-attention';
    }
    if (['Received', 'Dispatched', 'Approved', 'Inspected - good to go'].includes(order.status) || isOutboundOrder(order) || isPickupOrder(order)) return 'row-complete';
...
function applyTableDensity(density) {
    tableDensity = density === 'compact' ? 'compact' : 'comfortable';
    localStorage.setItem('max-logistics-table-density', tableDensity);
    document.body.classList.toggle('table-density-compact', tableDensity === 'compact');
    document.getElementById('btnDensityCompact')?.classList.toggle('active', tableDensity === 'compact');
    document.getElementById('btnDensityComfortable')?.classList.toggle('active', tableDensity !== 'compact');
}

function openColumnSettings() {
    closeAllModals();
    pendingColumnOrder = getSavedColumnOrder();
    renderColumnSettingsList();
    document.getElementById('modalColumnSettings').classList.remove('hidden');
}

function renderColumnSettingsList() {
    const labels = new Map(TABLE_COLUMNS[currentDashboardTab].map(column => [column[0], column[1]]));
    document.getElementById('columnSettingsList').innerHTML = pendingColumnOrder.map((key, index) => `
...
function openTaskCenter() {
    ...
    document.getElementById('modalTaskCenter').classList.remove('hidden');
}
...
function renderDashboard() {
    ...
    filteredOrders.forEach(order => {
        ...
        if (order.packing_slip_filename) {
            scanCell += `<span class="attachment-count"><i class="fa-solid fa-file-invoice"></i> Packing slip</span>`;
        }
        const podFiles = order.invoice_filenames || (order.invoice_filename ? [order.invoice_filename] : []);
...
function openGlobalSearch() {
    ...
    document.getElementById('modalGlobalSearch').classList.remove('hidden');
...
function setupEventListeners() {
    ...
    document.getElementById('btnMobileTasks')?.addEventListener('click', openTaskCenter);
    document.getElementById('btnColumnSettings')?.addEventListener('click', openColumnSettings);
    document.getElementById('btnSaveColumns')?.addEventListener('click', saveColumnSettings);
    document.getElementById('btnResetColumns')?.addEventListener('click', resetColumnSettings);
    document.getElementById('btnDensityComfortable')?.addEventListener('click', () => applyTableDensity('comfortable'));
    document.getElementById('btnDensityCompact')?.addEventListener('click', () => applyTableDensity('compact'));
...
function openWarrantyModal(orderId = null) {
    ...
    toggleWarrantyFields();
    document.getElementById('modalWarranty').classList.remove('hidden');
}
