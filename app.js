...
const DASHBOARD_TAB_STORAGE_KEY = 'max-logistics-active-dashboard-tab';
const DASHBOARD_TABS = ['receiving', 'outbound', 'pickup', 'trailers', 'warranty', 'inspections', 'stock'];
...
let pendingWarrantyTransfer = null;

let currentAdminSelectedCarrier = 'UPS';
let currentAdminRecordFilter = 'all';
...
const OUTBOUND_STATUSES = ['To be Shipped', 'Shipped'];
...
const TABLE_COLUMNS = {
    ...
    outbound: [
        ...
        ['status', 'Status', 'status'], ['photos', 'Photos']
    ],
    pickup: [
...
const STOCK_TABLE_COLUMNS = {
    doors: [
        ['reference', 'Model'], ['size', 'Size'], ['description', 'Description'], ['qty', 'QTY'], ['notes_count', 'Notes']
    ],
    operators: [
        ['reference', 'Model'], ['type', 'Type'], ['description', 'Description'], ['qty', 'QTY'], ['notes_count', 'Notes']
    ],
    parts: [
        ['reference', 'Model'], ['type', 'Type'], ['description', 'Description'], ['qty', 'QTY'], ['photos', 'Photo']
    ]
};
...
let globalSearchReturnState = null;
...
function setAuthenticatedUi(isAuthenticated) {
    ...
    ['btnDashboard', 'btnGlobalSearch', 'btnTaskCenter', 'btnMobileHeaderMore', 'btnMobileBottomMenu', 'btnMobileDashboard'].forEach(id => {
...
function getSavedDashboardTab() {
    ...
    return DASHBOARD_TABS.includes(savedTab) ? savedTab : 'receiving';
}

function restoreSavedDashboardTab() {
    setDashboardTab(getSavedDashboardTab(), { persist: false });
}
...
function isMobileLayoutViewport() {
    return window.matchMedia('(max-width: 900px), (max-width: 1024px) and (pointer: coarse)').matches;
}
...
function positionHeaderUserMenu() {
    const mobileTrigger = document.getElementById('btnMobileBottomMenu');
    const headerTrigger = document.getElementById('btnMobileHeaderMore');
    const mobileNav = document.getElementById('mobileBottomNav');
    const useBottomTrigger = isMobileLayoutViewport() && isElementVisible(mobileNav) && mobileTrigger;
    const trigger = useBottomTrigger ? mobileTrigger : headerTrigger;
    const menu = document.getElementById('headerUserMenu');
    if (!trigger || !menu || menu.classList.contains('hidden')) return;
    const menuHome = document.querySelector('.header-menu-wrapper');
    const rect = trigger.getBoundingClientRect();
    const menuWidth = Math.min(230, window.innerWidth - 24);
    menu.style.width = `${menuWidth}px`;
    if (trigger.id === 'btnMobileBottomMenu') {
        if (menu.parentElement !== document.body) document.body.appendChild(menu);
        menu.classList.add('mobile-menu-docked');
        ...
        menu.classList.remove('mobile-menu-docked');
...
function closeHeaderUserMenu() {
    ...
    document.getElementById('btnMobileBottomMenu')?.setAttribute('aria-expanded', 'false');
...
function updateThemeToggleIcon(theme) {
    const btnThemeToggle = document.getElementById('btnThemeToggle');
    if (btnThemeToggle) {
        btnThemeToggle.classList.toggle('active', theme === 'dark');
        btnThemeToggle.setAttribute('aria-pressed', String(theme === 'dark'));
        const icon = btnThemeToggle.querySelector('i');
        if (icon) {
            icon.className = theme === 'dark' ? 'fa-solid fa-sun' : 'fa-solid fa-circle-half-stroke';
        }
    }
    const headerThemeButton = document.getElementById('btnHeaderTheme');
    if (headerThemeButton) {
        headerThemeButton.classList.toggle('active', theme === 'dark');
        headerThemeButton.setAttribute('aria-pressed', String(theme === 'dark'));
    }
}
...
function getRecordDisplayStatus(order) {
    if (isOutboundOrder(order)) return getOutboundStatus(order);
    if (isPickupOrder(order)) return order.status || 'Picked Up';
    return order.status || 'Ordered';
}

function appendStatusHistory(order) {
    const status = getRecordDisplayStatus(order);
    if (!status) return;
...
function showToast(title, message = '', type = 'success', action = null) {
    ...
    toast.innerHTML = `<i class="fa-solid ${icon}"></i><div><strong>${escapeHtml(title)}</strong>${message ? `<span>${escapeHtml(message)}</span>` : ''}</div>${action ? `<button type="button" class="toast-action">${escapeHtml(action.label || 'Undo')}</button>` : ''}`;
    if (action?.handler) {
    ...
    window.setTimeout(() => toast.remove(), action ? 7200 : 3600);
}
...
function getOutboundStatus(order) {
    if (!isOutboundOrder(order)) return order?.status || '';
    return order.status === 'To be Shipped' ? 'To be Shipped' : 'Shipped';
}

function isOutboundComplete(order) {
    return isOutboundOrder(order) && getOutboundStatus(order) === 'Shipped';
}
...
function buildHighlightedSearchPreview(values, query) {
    const text = getMatchedSearchValues(values, query, 8).join(' | ');
...
function buildSearchDocumentBadges(documentHits = []) {
    ...
    return `<span class="global-search-doc-badges">${unique.map(type => `
        <span class="global-search-doc-badge ${type === 'stock' ? 'stock-hit' : ''}" title="${type === 'stock' ? 'Match found in Stock Remaining' : `Match found inside ${type === 'pdf' ? 'PDF' : 'Excel'} document`}">
            <i class="${type === 'stock' ? 'fa-solid fa-warehouse' : `fa-regular ${type === 'pdf' ? 'fa-file-pdf' : 'fa-file-excel'}`}"></i>${type === 'stock' ? 'Stock' : (type === 'pdf' ? 'PDF' : 'Excel')}
        </span>`).join('')}</span>`;
}
...
function getRecordSearchValues(order) {
    ...
    values.push(getRecordTypeLabel(order), getRecordReference(order), getRecordParty(order), getRecordDisplayStatus(order));
...
function getRecordSearchPreviewValues(order) {
    ...
    const add = (label, value) => {
        ...
        if (text && text !== '-') values.push(`${label}: ${text}`);
    };

    if (isTrailerOrder(order)) {
        add('Trailer', order.trailer_number);
        add('Weight', order.weight);
        add('Arrived', formatDateTimeDisplay(order.arrived_date));
        add('Dispatched', formatDateTimeDisplay(order.dispatched_date));
        if (order.issue_description) values.push('Issue reported');
        return values;
    }

    if (isOutboundOrder(order)) {
        add('Status', getOutboundStatus(order));
        add('Courier', carrierOptionLabel(order.carrier || 'LTL / Other'));
        add('Ship date', getOutboundStatus(order) === 'Shipped' ? formatDateTimeDisplay(order.shipped_date) : '');
        add('Payment', order.payment_type);
        add('Address', order.ship_to_address);
        return values;
    }

    if (isPickupOrder(order)) {
        add('Pick up date', formatDateTimeDisplay(order.pickup_date));
        add('Handled by', order.handled_by);
        add('Payment', order.payment_type);
        return values;
    }

    if (isWarrantyOrder(order)) {
        add('Order type', order.order_type);
        add('Claim type', order.claim_type);
        add('Original order', order.original_order_id);
        return values;
    }

    if (isInspectionOrder(order)) {
        add('Requested by', order.requested_by);
        add('Inspected by', order.inspected_by);
        add('Reason', order.reason);
        return values;
    }

    add('Courier', carrierOptionLabel(order.carrier || 'LTL / Other'));
    add('Ordered', formatDateTimeDisplay(order.ordered_date));
    add('ETA', order.eta);
    add('Received', formatDateTimeDisplay(order.received_date));
    if (order.osd) values.push('Issue reported');
    return values;
}
...
async function quickSetTrailerStatus(orderId, status) {
    ...
    if (!trailer) return;
    const previousTrailer = JSON.parse(JSON.stringify(trailer));
    ...
    try {
        ...
        await syncDatabase();
        showToast('Status updated', `Trailer marked ${status}.`, 'success', {
            label: 'Undo',
            handler: async () => {
                try {
                    await saveOrderRecord(previousTrailer);
                    await syncDatabase();
                    if (!document.getElementById('modalViewOrderDetails')?.classList.contains('hidden')) viewOrderDetails(orderId);
                    showToast('Status restored', `Back to ${previousTrailer.status}.`);
                } catch (err) {
                    showToast('Restore failed', 'Could not restore the previous status.', 'error');
                }
            }
        });
    } catch (err) {
...
async function quickSetWarrantyStatus(orderId, status) {
    ...
    const previousClaim = JSON.parse(JSON.stringify(claim));
    const previousStatus = claim.status || 'To be Claimed';
    const needsClaimDetails = ['Request Submitted', 'Approved', 'Denied'].includes(status);
    const missingProgress = needsClaimDetails && (!claim.original_order_id || !claim.customer_name || !claim.order_type);
    ...
    try {
        ...
        await syncDatabase();
        showToast('Status updated', `Warranty moved to ${status}.`, 'success', {
            label: 'Undo',
            handler: async () => {
                try {
                    await saveOrderRecord(previousClaim);
                    await syncDatabase();
                    if (!document.getElementById('modalViewOrderDetails')?.classList.contains('hidden')) viewOrderDetails(orderId);
                    showToast('Status restored', `Back to ${previousStatus}.`);
                } catch (err) {
                    showToast('Restore failed', 'Could not restore the previous status.', 'error');
                }
            }
...
function getReceivedEtaSummary(order) {
    if (!order?.eta || !order?.received_date) return null;
    const etaDate = parseDateOnly(order.eta);
    const receivedDate = parseDateOnly(order.received_date);
    if (!etaDate || !receivedDate) return null;
    const difference = Math.round((receivedDate - etaDate) / 86400000);
    if (difference < 0) return { status: 'early', label: `${Math.abs(difference)}d early` };
    if (difference === 0) return { status: 'on-time', label: 'On time' };
    return { status: 'late', label: `${difference}d late` };
}

function renderReceivedEtaBadge(order) {
    const summary = getReceivedEtaSummary(order);
    if (!summary) return '';
    const icon = summary.status === 'late' ? 'fa-triangle-exclamation' : (summary.status === 'early' ? 'fa-arrow-trend-down' : 'fa-circle-check');
    return `<span class="eta-arrival-badge ${summary.status}" title="ETA ${escapeHtml(order.eta)}"><i class="fa-solid ${icon}"></i>${escapeHtml(summary.label)}</span>`;
}
...
function sortReceivingOrders(a, b) {
    const statusRank = order => {
        if (order.status === 'Ordered' && order.eta) return 0;
        if (order.status === 'Ordered') return 1;
    ...
    if (rankA === 0) {
        const etaA = parseDateOnly(a.eta)?.getTime() ?? Number.MAX_SAFE_INTEGER;
        const etaB = parseDateOnly(b.eta)?.getTime() ?? Number.MAX_SAFE_INTEGER;
        if (etaA !== etaB) return etaA - etaB;
        return getDateTimeValue(b.ordered_date || b.created_date) - getDateTimeValue(a.ordered_date || a.created_date);
    }
    if (rankA === 1) return getDateTimeValue(b.ordered_date || b.created_date) - getDateTimeValue(a.ordered_date || a.created_date);
    if (rankA === 2) return getDateTimeValue(b.received_date) - getDateTimeValue(a.received_date);
    return (Number(b.id) || 0) - (Number(a.id) || 0);
}

function getWarrantyStatusRank(status) {
    if (status === 'To be Claimed') return 0;
    if (status === 'Working on it') return 1;
    if (status === 'Request Submitted') return 2;
    if (status === 'Approved' || status === 'Denied') return 3;
    return 4;
}

function sortDashboardDefaultOrders(a, b) {
    if (currentDashboardTab === 'receiving') return sortReceivingOrders(a, b);
    if (currentDashboardTab === 'outbound') {
        const rankA = getOutboundStatus(a) === 'To be Shipped' ? 0 : 1;
        const rankB = getOutboundStatus(b) === 'To be Shipped' ? 0 : 1;
        if (rankA !== rankB) return rankA - rankB;
        return getDateTimeValue(b.shipped_date || b.created_date) - getDateTimeValue(a.shipped_date || a.created_date);
    }
    if (currentDashboardTab === 'pickup') {
        return getDateTimeValue(b.pickup_date || b.created_date) - getDateTimeValue(a.pickup_date || a.created_date);
    }
    if (currentDashboardTab === 'warranty') {
        const rankA = getWarrantyStatusRank(a.status);
        const rankB = getWarrantyStatusRank(b.status);
        if (rankA !== rankB) return rankA - rankB;
        return getDateTimeValue(b.updated_date || b.created_date || b.id) - getDateTimeValue(a.updated_date || a.created_date || a.id);
    }
    const aComplete = TERMINAL_STATUSES.has(a.status) || isOutboundComplete(a) || isPickupOrder(a) ? 1 : 0;
    const bComplete = TERMINAL_STATUSES.has(b.status) || isOutboundComplete(b) || isPickupOrder(b) ? 1 : 0;
...
function getDuplicatePoSet(items) {
    const counts = new Map();
    items.filter(isReceivingOrder).forEach(order => {
        const key = String(order.po_number || '').trim().toLowerCase();
        if (!key) return;
...
function renderDuplicatePoBadge(order, duplicatePoSet) {
    const key = String(order.po_number || '').trim().toLowerCase();
    if (!key || !duplicatePoSet.has(key)) return '';
    return `<div class="duplicate-po-badge duplicate-po-icon-only" title="Duplicate PO in this list" aria-label="Duplicate PO"><i class="fa-solid fa-copy"></i></div>`;
}
...
function workflowStatusClass(status, order = null) {
    if (status === 'To be Claimed') return 'status-to-claim';
    if (status === 'Working on it') return 'status-working-on-it';
    if (status === 'To be Shipped') return 'status-pending';
    if (['Received', 'Dispatched', 'Approved', 'Inspected - good to go', 'Shipped Out', 'Shipped', 'Picked Up'].includes(status)) return 'status-complete';
    if (['Denied', 'Canceled', 'Inspected - damage found'].includes(status)) return 'status-problem';
    if (order?.has_issue) return 'status-problem';
    if (['Request Submitted', 'Arrived'].includes(status)) return 'status-working';
...
function getSavedStockColumnOrder(type = currentStockSubTab) {
    if (!STOCK_TABLE_COLUMNS[type]) return [];
    const defaults = STOCK_TABLE_COLUMNS[type].map(column => column[0]);
    try {
        const saved = JSON.parse(localStorage.getItem(`max-logistics-stock-columns-${type}`) || '[]');
        const valid = saved.filter(key => defaults.includes(key));
        defaults.filter(key => !valid.includes(key)).forEach(key => valid.push(key));
        return valid;
    } catch {
...
function renderStockTableHeader(type) {
    const tableHead = document.getElementById('stockTableHead');
    if (!tableHead || !STOCK_TABLE_COLUMNS[type]) return;
    const columnsByKey = new Map(STOCK_TABLE_COLUMNS[type].map(column => [column[0], column]));
    tableHead.innerHTML = `<tr>${getSavedStockColumnOrder(type).map(key => {
        const [, label] = columnsByKey.get(key);
        const align = ['qty', 'notes_count', 'photos'].includes(key) ? ' style="text-align:center;"' : '';
        return `<th data-column="${key}" data-column-scope="stock" draggable="true" class="draggable-column"${align}><span class="column-drag-label"><i class="fa-solid fa-grip-vertical"></i>${label}</span></th>`;
    }).join('')}</tr>`;
    setupDraggableColumns();
}

function recordRowState(order) {
    if (isOutboundOrder(order) && !isOutboundComplete(order)) return 'row-waiting';
    if (TERMINAL_STATUSES.has(order.status) || isOutboundComplete(order) || isPickupOrder(order)) return 'row-complete';
...
function appendConfiguredStockRow(tableBody, tr) {
    const cells = new Map(Array.from(tr.children).map(cell => [cell.dataset.column, cell]));
    getSavedStockColumnOrder(currentStockSubTab).forEach(key => cells.get(key) && tr.appendChild(cells.get(key)));
    tableBody.appendChild(tr);
}
...
function getTaskGroups() {
    return [
        { label: 'Shipped Out waiting to ship', icon: 'fa-paper-plane', items: orders.filter(order => isOutboundOrder(order) && getOutboundStatus(order) === 'To be Shipped') },
...
function syncMobileNavigation() {
    const menuOpen = !document.getElementById('headerUserMenu')?.classList.contains('hidden');
    const dashboardOpen = !document.getElementById('modalDashboard')?.classList.contains('hidden');
    document.querySelectorAll('[data-mobile-tab]').forEach(button => {
        button.classList.toggle('active', !menuOpen && !dashboardOpen && button.dataset.mobileTab === currentDashboardTab);
    });
    syncMobileUtilityButtonStates();
}

function syncMobileUtilityButtonStates() {
    const dashboardOpen = !document.getElementById('modalDashboard')?.classList.contains('hidden');
    const menuOpen = !document.getElementById('headerUserMenu')?.classList.contains('hidden');
    document.getElementById('btnMobileBottomMenu')?.classList.toggle('active', Boolean(menuOpen));
    document.getElementById('btnMobileDashboard')?.classList.toggle('active', !menuOpen && Boolean(dashboardOpen));
}

function closeMobileOverlayBeforeSliderAction({ keepMenu = false, keepDashboard = false } = {}) {
    if (!isMobileLayoutViewport()) return true;
    if (!keepDashboard) {
        if (closeAllModals() === false) return false;
    } else {
        if (!confirmDiscardModalChanges()) return false;
        document.querySelectorAll('.modal-overlay:not(.hidden)').forEach(modal => {
            if (modal.id === 'modalDashboard') return;
            modal.classList.add('hidden');
            modalOriginalStates.delete(modal.id);
        });
        activeStockDetailType = null;
        const remainingOpenModals = document.querySelectorAll('.modal-overlay:not(.hidden)').length;
        if (remainingOpenModals === 0) unlockPageScroll();
        else modalLockDepth = remainingOpenModals;
    }
    if (!keepMenu) closeHeaderUserMenu();
    if (!keepDashboard) closeModalById('modalDashboard');
    syncMobileNavigation();
    return true;
}
...
function triggerMobileCreate() {
    document.getElementById('mobileMorePanel')?.classList.add('hidden');
    if (!closeMobileOverlayBeforeSliderAction()) return;
...
function closeModalById(modalId) {
    ...
    modal.classList.add('hidden');
    modalOriginalStates.delete(modalId);
    const remainingOpenModals = document.querySelectorAll('.modal-overlay:not(.hidden)').length;
    if (remainingOpenModals === 0) unlockPageScroll();
    else modalLockDepth = remainingOpenModals;
    syncMobileNavigation();
...
function updateDashboardChrome() {
    ...
    document.getElementById('statusFilterWrapper')?.classList.toggle('hidden', isStock || !(isReceiving || isOutbound || isWarranty || isInspections));
...
function setDashboardTab(tab, options = {}) {
    if (!DASHBOARD_TABS.includes(tab)) tab = 'receiving';
    if (options.persist !== false) localStorage.setItem(DASHBOARD_TAB_STORAGE_KEY, tab);
...
function renderDashboard() {
    ...
    const filteredOrders = orders.filter(order => {
        ...
        // Status filter
        if (isOutboundTab) {
            if (filterStatus !== 'all' && getOutboundStatus(order) !== filterStatus) return false;
        } else if (!isPickupTab && !isTrailersTab && filterStatus !== 'all' && order.status !== filterStatus) {
            return false;
        }
        ...
        if (searchVal) {
            const searchableFields = isTrailersTab
                ...
                : (isOutboundTab
                    ? [order.customer, order.job_number, getOutboundStatus(order), order.carrier, order.tracking_number, order.ship_to_address, order.payment_type, order.notes]
    ...
    if (!currentSortColumn) filteredOrders.sort(sortDashboardDefaultOrders);
    currentDashboardExportRows = [...filteredOrders];
    const duplicatePoSet = getDuplicatePoSet(filteredOrders);
    ...
    if (filteredOrders.length === 0) {
        ...
        const emptyIcon = document.querySelector('#emptyState .empty-state-icon i');
        if (emptyIcon) emptyIcon.className = `fa-solid ${filtered ? 'fa-magnifying-glass' : 'fa-inbox'}`;
    ...
    filteredOrders.forEach(order => {
        ...
        if (isOutboundTab) {
            ...
            const outboundStatus = getOutboundStatus(order);
            ...
            tr.innerHTML = `
                ...
                <td><span class="badge ${workflowStatusClass(outboundStatus, order)}">${escapeHtml(outboundStatus)}</span></td>
                <td>${photoCell}</td>
            `;
            appendConfiguredRow(tableBody, tr, order);
            return;
        ...
        if (order.status === 'Received' && order.received_by && order.received_date) {
            statusCell += `<div class="received-info">by ${escapeHtml(order.received_by)}<br>${formatDateTimeDisplay(order.received_date)}<br>${renderReceivedEtaBadge(order)}</div>`;
        }
        if (order.has_issue) {
        ...
        tr.innerHTML = `
            <td class="po-col">
                ...
                ${renderDuplicatePoBadge(order, duplicatePoSet)}
                ${classificationBadge}
            </td>
...
function updateStats() {
    ...
    orders.filter(isOutboundOrder).forEach(order => {
        if (isOutboundComplete(order) && order.shipped_date && order.shipped_date.split('T')[0] === todayStr) shippedTodayCount++;
    });
...
function renderOperationsDashboard() {
    ...
    const shippedOrders = orders.filter(order => isOutboundComplete(order) && isDateInDashboardRange(order.shipped_date, range));
...
function setupDraggableColumns() {
    ...
    headers.forEach(header => {
        ...
        header.addEventListener('drop', event => {
            ...
            const isStockColumn = header.dataset.columnScope === 'stock';
            const order = isStockColumn ? getSavedStockColumnOrder(currentStockSubTab) : getSavedColumnOrder();
            const from = order.indexOf(draggedColumnKey);
            const to = order.indexOf(targetKey);
            if (from === -1 || to === -1) return;
            order.splice(to, 0, order.splice(from, 1)[0]);
            if (isStockColumn) {
                localStorage.setItem(`max-logistics-stock-columns-${currentStockSubTab}`, JSON.stringify(order));
            } else {
                localStorage.setItem(`max-logistics-columns-${currentDashboardTab}`, JSON.stringify(order));
            }
            columnDragStarted = true;
            const tableBody = document.getElementById(isStockColumn ? 'stockTableBody' : 'ordersTableBody');
            tableBody?.classList.add('refreshing');
            if (isStockColumn) renderStockTable();
...
function openGlobalSearch(options = {}) {
    ...
    const restoreQuery = options.preserve && globalSearchReturnState?.query ? globalSearchReturnState.query : '';
    if (input) input.value = restoreQuery;
    if (!restoreQuery) {
        globalSearchReturnState = null;
        document.getElementById('globalSearchCount').textContent = '0 results';
        document.getElementById('globalSearchResults').innerHTML = '<div class="global-search-empty apple-empty"><i class="fa-solid fa-keyboard"></i><strong>Search across the entire portal</strong><span>Enter at least two characters to begin.</span></div>';
    } else {
        renderGlobalSearch();
    }
...
function renderGlobalSearchLegacy() {
    ...
    if (query.length < 2) {
        ...
        results.innerHTML = '<div class="global-search-empty apple-empty"><i class="fa-solid fa-keyboard"></i><strong>Search across the entire portal</strong><span>Enter at least two characters to begin.</span></div>';
        return;
    }
    ...
    if (!matches.length) {
        results.innerHTML = '<div class="global-search-empty apple-empty"><i class="fa-solid fa-magnifying-glass"></i><strong>No matching records</strong><span>Try a PO, Job, BOL, customer, courier, or document phrase.</span></div>';
        return;
    }
        ...
        <section class="global-search-group">
            ...
            ${records.slice(0, 12).map(order => `
                <button type="button" class="global-search-result" onclick="openGlobalSearchResult('${order.id}')">
                    ...
                    <span class="badge ${workflowStatusClass(getRecordDisplayStatus(order), order)}">${escapeHtml(getRecordDisplayStatus(order))}</span>
                </button>`).join('')}
        </section>`).join('');
}

function openGlobalSearchResult(orderId) {
    globalSearchReturnState = {
...
function renderGlobalSearch() {
    ...
    const orderMatches = orders
        ...
        .map(order => {
            ...
            return {
                ...
                previewHtml: buildHighlightedSearchPreview(previewValues, query) || 'Open details',
    ...
    const stockMatches = getAllStockSearchRecords()
        ...
        .map(record => ({
            ...
            party: record.party,
            previewHtml: buildHighlightedSearchPreview(record.previewValues, query) || 'Open details',
    ...
    count.textContent = `${matches.length} result${matches.length === 1 ? '' : 's'}`;
    if (!matches.length) {
        results.innerHTML = '<div class="global-search-empty apple-empty"><i class="fa-solid fa-magnifying-glass"></i><strong>No matching records</strong><span>Try a PO, Job, BOL, customer, courier, stock item, comment, file name, or document phrase.</span></div>';
        return;
    }
        ...
        <section class="global-search-group">
            ...
            ${records.slice(0, 12).map(record => `
                <button type="button" class="global-search-result" onclick="${record.onclick}">
                    ...
                    <span class="badge apple-status ${record.statusClass}">${escapeHtml(record.status)}</span>
                </button>`).join('')}
        </section>`).join('');
}
...
function closeDetailAndReturnToGlobalSearch() {
    const shouldReturn = Boolean(globalSearchReturnState?.query);
    if (closeAllModals() === false) return false;
    if (shouldReturn) {
        window.setTimeout(() => openGlobalSearch({ preserve: true }), 40);
    }
    return true;
}
...
function getRecordActionButtons(order) {
    ...
    } else if (isInspectionOrder(order) && order.status === 'Inspected - damage found') {
        buttons.push(`<button class="btn btn-success" onclick="createWarrantyFromInspection('${order.id}')"><i class="fa-solid fa-shield-halved"></i> Create Warranty</button>`);
    }
...
function renderRecordHero(order) {
    ...
    if (isReceivingOrder(order)) {
        ...
        meta.push(['fa-truck-fast', getOutboundStatus(order) === 'Shipped' ? `Shipped ${formatDateTimeDisplay(order.shipped_date)}` : 'To be Shipped']);
...
function renderFileTile({ title, count = 0, icon = 'fa-paperclip', view, add, tone = '', previewOrderId = '', previewFile = null }) {
    ...
    const tileOpen = hasPreview && view;
    ...
    return `<div class="record-file-tile ${hasFiles ? '' : 'empty'} ${tileOpen ? 'clickable' : ''} ${tone}" ${tileOpen ? `role="button" tabindex="0" onclick="${view}" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();${view}}"` : ''}>
        ...
        <div class="record-file-actions">
            ${hasFiles && view ? `<button class="btn btn-secondary btn-icon${previewClass}"${previewData} onclick="event.stopPropagation();${view}" title="View"><i class="fa-solid fa-eye"></i></button>` : ''}
            ${add ? `<button class="btn btn-secondary btn-icon" onclick="event.stopPropagation();${add}" title="Add"><i class="fa-solid fa-plus"></i></button>` : ''}
        </div>
    </div>`;
}
...
function getRecordPdfFiles(order) {
    if (isTrailerOrder(order)) {
        return (order.documents || [])
            .filter(file => isPdfFilename(file.filename))
            .map((file, index) => ({
                folder: 'clopay_trailers',
                filename: file.filename,
                originalName: file.original_name || file.filename,
                galleryType: 'trailer_document',
    ...
    const packingSlipFiles = getPackingSlipFiles(order)
        ...
        .map((filename, index) => ({
            folder: 'scans',
            filename,
            galleryType: 'packing_slip',
            title: `Attached PDF ${index + 1}`
        }));
    const podFiles = (order.invoice_filenames || (order.invoice_filename ? [order.invoice_filename] : []))
        .filter(isPdfFilename)
        .map((filename, index) => ({
            ...
            filename,
            galleryType: 'pod',
            title: `POD PDF ${index + 1}`
        }));
    return [...packingSlipFiles, ...podFiles];
}

function getRecordPreviewFiles(order) {
    return [
        ...getRecordPhotoFiles(order).map(file => ({ ...file, kind: 'photo' })),
        ...getRecordPdfFiles(order).map(file => ({ ...file, kind: 'pdf' }))
    ];
}

function photoAnnotationKey(file) {
    return `${file.folder}/${file.filename}`;
}

function recordHasPhotoAnnotation(order, file) {
    return Boolean(order?.photo_annotations?.[photoAnnotationKey(file)]);
}

async function getPdfPreviewThumbnail(file) {
    const key = `${file.folder}/${file.filename}`;
    if (pdfPreviewCache.has(key)) return pdfPreviewCache.get(key);
    ...
    try {
        ...
        pdfPreviewCache.set(key, thumbUrl);
        return thumbUrl;
    } finally {
        pdf.destroy?.();
    }
...
async function renderDetailPhotoPreviewStrip(order) {
    ...
    if (!container) return;
    const allPreviews = getRecordPreviewFiles(order);
    const previews = allPreviews.slice(0, 12);
    if (!previews.length) {
        container.innerHTML = `<div class="record-photo-empty"><i class="fa-regular fa-images"></i><span>No photos or PDF files yet</span></div>`;
        return;
    }
    const thumbs = await Promise.all(previews.map(async file => {
        const key = `${file.folder}/${file.filename}`;
        const title = file.kind === 'pdf' ? 'Open PDF' : 'Open photo';
        const badge = file.kind === 'pdf'
            ? '<span class="record-file-kind-badge"><i class="fa-regular fa-file-pdf"></i> PDF</span>'
            : (recordHasPhotoAnnotation(order, file) ? '<span class="record-annotation-badge"><i class="fa-solid fa-pen-nib"></i> Annotated</span>' : '');
        try {
            if (file.kind === 'pdf') {
                const pdfThumb = await getPdfPreviewThumbnail(file);
                return `<button type="button" class="record-preview-thumb-btn record-preview-pdf-btn" data-preview-folder="${escapeHtml(file.folder)}" data-preview-filename="${escapeHtml(file.filename)}" onclick="openRecordAttachmentPreview('${escapeJsString(order.id)}', '${escapeJsString(file.filename)}')" title="${title}">
                    ${pdfThumb ? `<img class="record-preview-thumb" src="${pdfThumb}" alt="Open PDF">` : '<span class="record-preview-pdf-fallback"><i class="fa-regular fa-file-pdf"></i></span>'}
                    ${badge}
                </button>`;
            }
            if (!photoPreviewCache.has(key)) {
            ...
            return `<button type="button" class="record-preview-thumb-btn photo-preview-trigger" data-preview-folder="${escapeHtml(file.folder)}" data-preview-filename="${escapeHtml(file.filename)}" onclick="openRecordAttachmentPreview('${escapeJsString(order.id)}', '${escapeJsString(file.filename)}')" title="${title}">
                <img class="record-preview-thumb" src="${photoPreviewCache.get(key)}" alt="Open photo">
                ${badge}
            </button>`;
        } catch {
            if (file.kind !== 'pdf') return '';
            return `<button type="button" class="record-preview-thumb-btn record-preview-pdf-btn" onclick="openRecordAttachmentPreview('${escapeJsString(order.id)}', '${escapeJsString(file.filename)}')" title="${title}">
                <span class="record-preview-pdf-fallback"><i class="fa-regular fa-file-pdf"></i></span>
                ${badge}
            </button>`;
        }
    }));
    const photoCount = allPreviews.filter(file => file.kind === 'photo').length;
    const pdfCount = allPreviews.filter(file => file.kind === 'pdf').length;
    const countLabel = [photoCount ? `${photoCount} photo${photoCount === 1 ? '' : 's'}` : '', pdfCount ? `${pdfCount} PDF${pdfCount === 1 ? '' : 's'}` : ''].filter(Boolean).join(' · ');
    const remaining = allPreviews.length - previews.length;
    container.innerHTML = `<div class="record-photo-strip-head"><span>${countLabel}</span></div><div class="record-photo-strip-scroller">${thumbs.join('')}${remaining > 0 ? `<button type="button" class="record-preview-more" onclick="openRecordAttachmentPreview('${escapeJsString(order.id)}', '${escapeJsString(previews[0]?.filename || '')}')">+${remaining}</button>` : ''}</div>`;
}

function ensureEditAttachmentPreview(containerId, afterElementId) {
    ...
    container.className = 'edit-attachment-preview-strip';
    anchor.insertAdjacentElement('afterend', container);
    return container;
}

async function renderEditAttachmentPreviews(order, containerId, files) {
    const container = document.getElementById(containerId);
    if (!container) return;
    const previews = (files || []).filter(Boolean).slice(0, 12);
    if (!order || !previews.length) {
        container.classList.add('hidden');
        container.innerHTML = '';
        return;
    }
    container.classList.remove('hidden');
    container.innerHTML = '<div class="edit-attachment-loading"><i class="fa-solid fa-spinner fa-spin"></i><span>Loading previews...</span></div>';
    const thumbs = await Promise.all(previews.map(async file => {
        const kind = file.kind || (isPdfFilename(file.filename) ? 'pdf' : 'photo');
        const title = kind === 'pdf' ? 'Open PDF' : 'Open photo';
        const badge = kind === 'pdf'
            ? '<span class="record-file-kind-badge"><i class="fa-regular fa-file-pdf"></i> PDF</span>'
            : '';
        try {
            if (kind === 'file') {
                return `<button type="button" class="record-preview-thumb-btn record-preview-pdf-btn" onclick="viewTrailerDocuments('${escapeJsString(order.id)}')" title="${escapeHtml(file.originalName || file.filename)}">
                    <span class="record-preview-pdf-fallback"><i class="fa-regular fa-file-excel"></i></span>
                    <span class="record-file-kind-badge"><i class="fa-regular fa-file-excel"></i> Excel</span>
                </button>`;
            }
            if (kind === 'pdf') {
                const pdfThumb = await getPdfPreviewThumbnail(file);
            ...
            return `<button type="button" class="record-preview-thumb-btn record-preview-pdf-btn" onclick="openRecordAttachmentPreview('${escapeJsString(order.id)}', '${escapeJsString(file.filename)}')" title="${escapeHtml(file.originalName || file.filename)}">
                <span class="record-preview-pdf-fallback"><i class="fa-regular ${kind === 'pdf' ? 'fa-file-pdf' : 'fa-file-image'}"></i></span>
                ${badge}
            </button>`;
        }
    }));
    const remaining = Math.max(0, (files || []).length - previews.length);
    container.innerHTML = `<div class="record-photo-strip-head"><span>${files.length} attached</span></div><div class="record-photo-strip-scroller">${thumbs.join('')}${remaining > 0 ? `<span class="record-preview-more">+${remaining}</span>` : ''}</div>`;
}

function renderSelectedFilePreviews(containerId, files) {
    const container = document.getElementById(containerId);
    if (!container) return;
    const selected = Array.from(files || []).filter(Boolean);
    if (!selected.length) {
        renderEditAttachmentPreviews(null, containerId, []);
        return;
    }
    container.classList.remove('hidden');
    const thumbs = selected.slice(0, 12).map(file => {
        const isImage = file.type?.startsWith('image/');
        const isPdf = file.type === 'application/pdf' || isPdfFilename(file.name);
        const src = isImage ? URL.createObjectURL(file) : '';
        const icon = isPdf ? 'fa-file-pdf' : 'fa-file-lines';
        const label = isPdf ? 'PDF' : (isImage ? 'Photo' : 'File');
        return `<span class="record-preview-thumb-btn record-preview-pdf-btn" title="${escapeHtml(file.name)}">
            ${src ? `<img class="record-preview-thumb" src="${src}" alt="${escapeHtml(file.name)}">` : `<span class="record-preview-pdf-fallback"><i class="fa-regular ${icon}"></i></span>`}
            <span class="record-file-kind-badge">${label}</span>
        </span>`;
    }).join('');
    const remaining = Math.max(0, selected.length - 12);
    container.innerHTML = `<div class="record-photo-strip-head"><span>${selected.length} selected</span></div><div class="record-photo-strip-scroller">${thumbs}${remaining > 0 ? `<span class="record-preview-more">+${remaining}</span>` : ''}</div>`;
}
...
function renderFilesSection(order) {
    ...
    if (isReceivingOrder(order)) {
        ...
        const podFiles = order.invoice_filenames || (order.invoice_filename ? [order.invoice_filename] : []);
        tiles = [
            renderFileTile({ title: 'Attached Files', count: packingSlipFiles.length, icon: 'fa-file-invoice', view: `viewFileAttachment('${order.id}', 'packing_slip')`, add: `addFilesDirectly('${order.id}', 'receiving_attached')`, previewFile: getFirstImagePreviewFile('scans', packingSlipFiles) })
        ];
        if (order.status === 'Received' && podFiles.length > 0) {
            tiles.push(renderFileTile({ title: 'POD / Photos', count: podFiles.length, icon: 'fa-file-signature', view: `viewFileAttachment('${order.id}', 'pod')`, add: `addFilesDirectly('${order.id}', 'receiving_pod')`, previewFile: getFirstImagePreviewFile('scans', podFiles) }));
        }
        if (order.has_issue) {
            tiles.push(renderFileTile({ title: 'OSD Photos', count: order.osd_photos?.length || 0, icon: 'fa-camera', view: `viewOsdPhotos('${order.id}')`, add: `addFilesDirectly('${order.id}', 'receiving_osd')`, tone: 'record-issue-block', previewFile: getFirstImagePreviewFile('osd', order.osd_photos || []) }));
        }
    } else if (isTrailerOrder(order)) {
        tiles = [
            renderFileTile({ title: 'Documents', count: order.documents?.length || 0, icon: 'fa-paperclip', view: `viewTrailerDocuments('${order.id}')`, add: `addFilesDirectly('${order.id}', 'trailer_docs')` })
        ];
        if (order.has_issue) {
            tiles.push(renderFileTile({ title: 'Issue Photos', count: order.issue_photos?.length || 0, icon: 'fa-camera', view: `viewTrailerIssuePhotos('${order.id}')`, add: `addFilesDirectly('${order.id}', 'trailer_issue_photos')`, tone: 'record-issue-block', previewFile: getFirstImagePreviewFile('clopay_issues', order.issue_photos || []) }));
        }
    } else {
        ...
        tiles = [renderFileTile({ title: 'Photos', count, icon: 'fa-camera', view: `${viewer}('${order.id}')`, add: `addFilesDirectly('${order.id}', '${kind}')`, previewFile: getFirstImagePreviewFile(folder, filenames), previewOrderId })];
    }
    return renderDetailSection('Files & photos', 'fa-images', `<div class="record-preview-strip" data-detail-previews="${order.id}"></div><div class="record-file-grid record-file-grid-compact">${tiles.join('')}</div>`);
}
...
function getActionTimeFields(order) {
    ...
    if (isOutboundOrder(order)) return [{ key: 'shipped_date', label: 'Shipped', statuses: ['Shipped', 'Shipped Out'] }];
...
function openHelpCenter() {
    ...
    document.querySelectorAll('.help-nav-btn').forEach(button => button.classList.remove('active'));
...
function setupEventListeners() {
    ...
    document.getElementById('btnMobileBottomMenu')?.addEventListener('click', event => {
        const menu = document.getElementById('headerUserMenu');
        const isOpen = menu?.classList.toggle('hidden') === false;
        event.currentTarget.setAttribute('aria-expanded', String(isOpen));
        document.getElementById('btnMobileHeaderMore')?.setAttribute('aria-expanded', String(isOpen));
        if (isOpen) positionHeaderUserMenu();
        document.getElementById('mobileMorePanel')?.classList.add('hidden');
        syncMobileNavigation();
    });
    ...
    document.getElementById('btnHeaderTheme')?.addEventListener('click', event => {
    ...
    document.addEventListener('click', event => {
        if (!event.target.closest('.header-menu-wrapper') && !event.target.closest('#btnMobileBottomMenu')) {
            closeHeaderUserMenu();
        }
    });
    ...
    document.getElementById('btnMobileDashboard')?.addEventListener('click', () => {
        document.getElementById('mobileMorePanel')?.classList.add('hidden');
        if (!closeMobileOverlayBeforeSliderAction({ keepDashboard: true })) return;
        openOperationsDashboard();
    });
    ...
    document.getElementById('mobileTabSelect')?.addEventListener('change', event => {
        if (!closeMobileOverlayBeforeSliderAction()) return;
        setDashboardTab(event.target.value);
    });
    ...
    document.getElementById('formStatus')?.addEventListener('change', () => syncOrderDateTimeControl());
    document.getElementById('formOrderCurrentDateToggle')?.addEventListener('click', () => {
        const enabled = !isOrderCurrentDateEnabled();
        setOrderCurrentDateMode(enabled);
        syncOrderDateTimeControl();
    });
    ...
    const configureWorkflowPhotos = (prefix, setSelection) => {
        ...
        const acceptFiles = files => {
            ...
            renderSelectedFilePreviews(`${prefix}EditAttachmentPreviews`, images);
        };
        ...
        remove?.addEventListener('click', () => {
            ...
            renderEditAttachmentPreviews(null, `${prefix}EditAttachmentPreviews`, []);
        });
    };
    ...
    document.querySelectorAll('.modal-close-btn').forEach(btn => {
        btn.addEventListener('click', event => {
            if (globalSearchReturnState?.query && btn.closest('#modalViewOrderDetails, #modalStockDetail')) {
                event.preventDefault();
                closeDetailAndReturnToGlobalSearch();
                return;
            }
            if (btn.closest('#modalGlobalSearch')) globalSearchReturnState = null;
            closeAllModals();
        });
    });
    document.querySelectorAll('.gallery-viewer-close-btn').forEach(btn => {
        btn.addEventListener('click', closeViewerModal);
    });
    DIRTY_GUARD_MODAL_IDS.forEach(modalId => {
    ...
    document.getElementById('modalViewOrderDetails')?.addEventListener('click', event => {
        if (event.target === event.currentTarget && !isMobileLayoutViewport()) {
            if (globalSearchReturnState?.query) closeDetailAndReturnToGlobalSearch();
            else closeAllModals();
        }
    });
    document.getElementById('modalViewScan')?.addEventListener('click', event => {
        if (event.target === event.currentTarget) closeViewerModal();
    });
    ['modalDashboard', 'modalGlobalSearch', 'modalTaskCenter', 'modalHelp'].forEach(modalId => {
        document.getElementById(modalId)?.addEventListener('click', event => {
            if (event.target === event.currentTarget && !isMobileLayoutViewport()) closeAllModals();
        });
    });
    document.getElementById('modalStockAnalytics')?.addEventListener('click', event => {
        if (event.target === event.currentTarget) closeAllModals();
    });
    ['modalStockDetail', 'modalStockExtras', 'modalNonStandardDoors'].forEach(modalId => {
        document.getElementById(modalId)?.addEventListener('click', event => {
            if (event.target === event.currentTarget) {
                if (modalId === 'modalStockDetail' && globalSearchReturnState?.query) closeDetailAndReturnToGlobalSearch();
                else closeAllModals();
            }
        });
    });
    ...
    document.getElementById('btnDeleteGalleryPhoto')?.addEventListener('click', deleteCurrentGalleryPhoto);
...
function closeViewerModal() {
    ...
    modal.classList.add('hidden');
    modalOriginalStates.delete('modalViewScan');
    ...
    const deleteButton = document.getElementById('btnDeleteGalleryPhoto');
    if (deleteButton) {
    ...
    document.querySelector('.gallery-modal-content')?.classList.remove('pdf-active');
    document.getElementById('modalViewScan')?.classList.remove('pdf-document-active');
...
function isOrderCurrentDateEnabled() {
    return document.getElementById('formOrderCurrentDateToggle')?.classList.contains('active') ?? true;
}

function setOrderCurrentDateMode(enabled, value = '') {
    const toggle = document.getElementById('formOrderCurrentDateToggle');
    ...
    dateInput.classList.toggle('current-datetime-locked', enabled);
...
function syncOrderDateTimeControl() {
    const status = document.getElementById('formStatus')?.value;
    const label = document.getElementById('formOrderDateLabel');
    const dateInput = document.getElementById('formOrderedDate');
    if (label) {
        label.textContent = status === 'Received' ? 'Date & Time Received *' : 'Date & Time Ordered *';
    }
    if (dateInput && isOrderCurrentDateEnabled()) {
        dateInput.value = getLocalDateTimeString();
        dateInput.disabled = true;
    }
}

function getOrderActionDateValue() {
    const dateInput = document.getElementById('formOrderedDate');
    if (!dateInput) return getLocalDateTimeString();
    if (isOrderCurrentDateEnabled()) {
        dateInput.value = getLocalDateTimeString();
    }
...
function openOrderModal(orderToEdit = null) {
    ...
    if (orderToEdit) {
        ...
        const editDateValue = document.getElementById('formStatus').value === 'Received'
            ? (orderToEdit.received_date || orderToEdit.ordered_date || getLocalDateTimeString())
            : (orderToEdit.ordered_date || getLocalDateTimeString());
        setOrderCurrentDateMode(false, editDateValue);
        ...
        if (packingSlipFiles.length) {
            ...
            renderEditAttachmentPreviews(orderToEdit, 'orderEditAttachmentPreviews', packingSlipFiles.map(filename => ({
                folder: 'scans',
                filename,
                kind: isPdfFilename(filename) ? 'pdf' : 'photo'
            })));
        } else {
            dropZone.classList.remove('hidden');
            selectStatus.classList.add('hidden');
            renderEditAttachmentPreviews(null, 'orderEditAttachmentPreviews', []);
        }
    } else {
        title.textContent = 'Create New Shipment';
        hiddenId.value = '';
        document.getElementById('formCarrier').value = '';
        setOrderCurrentDateMode(true);
    }
...
function resetOutboundPhotoVisuals() {
    ...
    renderEditAttachmentPreviews(null, 'outboundEditAttachmentPreviews', []);
}
...
function resetPickupPhotoVisuals() {
    ...
    renderEditAttachmentPreviews(null, 'pickupEditAttachmentPreviews', []);
}
...
function openOutboundModal(orderId = null) {
    ...
    if (outbound) {
        ...
        document.getElementById('outboundStatus').value = getOutboundStatus(outbound);
        ...
        if (outbound.shipped_photos && outbound.shipped_photos.length > 0) {
            ...
            renderEditAttachmentPreviews(outbound, 'outboundEditAttachmentPreviews', outbound.shipped_photos.map(filename => ({ folder: 'shipped_out', filename, kind: 'photo' })));
        }
    } else {
        document.getElementById('outboundStatus').value = 'Shipped';
    }
...
function openPickupModal(orderId = null) {
    ...
    if (pickup) {
        ...
        if (pickup.pickup_photos && pickup.pickup_photos.length > 0) {
            ...
            renderEditAttachmentPreviews(pickup, 'pickupEditAttachmentPreviews', pickup.pickup_photos.map(filename => ({ folder: 'customer_pickup', filename, kind: 'photo' })));
        }
    }
...
function resetTrailerUploadVisuals() {
    ...
    renderEditAttachmentPreviews(null, 'trailerDocsEditAttachmentPreviews', []);
    renderEditAttachmentPreviews(null, 'trailerIssueEditAttachmentPreviews', []);
}
...
function openTrailerModal(orderId = null) {
    ...
    if (trailer) {
        ...
        if (trailer.documents?.length) {
            ...
            renderEditAttachmentPreviews(trailer, 'trailerDocsEditAttachmentPreviews', trailer.documents.map(file => ({
                folder: 'clopay_trailers',
                filename: file.filename,
                originalName: file.original_name || file.filename,
                kind: isPdfFilename(file.filename) ? 'pdf' : 'file'
            })));
        }
        if (trailer.issue_photos?.length) {
            ...
            renderEditAttachmentPreviews(trailer, 'trailerIssueEditAttachmentPreviews', trailer.issue_photos.map(filename => ({ folder: 'clopay_issues', filename, kind: 'photo' })));
        }
    }
...
function openReceiveModal(orderId) {
    ...
    const existingPodFiles = order.invoice_filenames || (order.invoice_filename ? [order.invoice_filename] : []);
    if (existingPodFiles.length) {
        document.getElementById('dropZone')?.classList.remove('hidden');
        document.getElementById('fileSelectionStatus')?.classList.remove('hidden');
        const podName = document.getElementById('selectedFileName');
        if (podName) podName.textContent = `Attached: ${existingPodFiles.length} POD file${existingPodFiles.length === 1 ? '' : 's'}`;
        renderEditAttachmentPreviews(order, 'receivePodEditAttachmentPreviews', existingPodFiles.map(filename => ({
            folder: 'scans',
            filename,
            kind: isPdfFilename(filename) ? 'pdf' : 'photo'
        })));
    ...
    if (hasExistingIssue) {
        ...
        if (order.osd_photos?.length) {
            document.getElementById('osdDropZone')?.classList.remove('hidden');
            document.getElementById('osdFileSelectionStatus')?.classList.remove('hidden');
            const osdName = document.getElementById('osdSelectedFilesName');
            if (osdName) osdName.textContent = `Attached: ${order.osd_photos.length} OSD photo${order.osd_photos.length === 1 ? '' : 's'}`;
            renderEditAttachmentPreviews(order, 'receiveOsdEditAttachmentPreviews', order.osd_photos.map(filename => ({ folder: 'osd', filename, kind: 'photo' })));
        }
    }
...
function resetPodVisuals() {
    ...
    renderEditAttachmentPreviews(null, 'receivePodEditAttachmentPreviews', []);
}
...
function resetOsdDragAndDropVisuals() {
    ...
    clearOsdThumbnails();
    renderEditAttachmentPreviews(null, 'receiveOsdEditAttachmentPreviews', []);
}
...
async function handleOrderSubmit(e) {
    ...
    const actionDate = getOrderActionDateValue();
    ...
    order.ordered_date = status === 'Received' ? (order.ordered_date || actionDate) : actionDate;
    order.eta = eta;
    ...
    if (status === 'Received') {
        order.received_date = actionDate;
        order.received_by = order.received_by || orderedBy;
        const etaSummary = getReceivedEtaSummary(order);
        order.eta_arrival_status = etaSummary?.status || null;
        order.eta_arrival_label = etaSummary?.label || null;
    } else {
        order.eta_arrival_status = null;
        order.eta_arrival_label = null;
    }
...
async function handleOutboundSubmit(e) {
    ...
    const status = document.getElementById('outboundStatus')?.value || 'Shipped';
    ...
    outbound.status = OUTBOUND_STATUSES.includes(status) ? status : 'Shipped';
    ...
    if (outbound.status === 'Shipped' && !outbound.shipped_date) outbound.shipped_date = getLocalDateTimeString();
    if (outbound.status === 'To be Shipped') outbound.shipped_date = '';
...
function toggleWarrantyFields() {
    ...
    document.getElementById('warrantyOrderType').required = needsClaimDetails;
    document.getElementById('warrantyOriginalOrderId').required = needsClaimDetails;
    document.getElementById('warrantyCustomerName').required = needsProgress;
    document.getElementById('warrantyOrderTypeLabel').textContent = `Order Type${needsClaimDetails ? ' *' : ''}`;
    document.getElementById('warrantyOriginalOrderIdLabel').textContent = `Original Order ID${needsClaimDetails ? ' *' : ''}`;
    document.getElementById('warrantyNotes').required = false;
...
function openWarrantyModal(orderId = null, transferSourceId = null) {
    if (closeAllModals() === false) return;
    pendingWarrantyTransfer = transferSourceId ? { sourceInspectionId: transferSourceId } : null;
    const form = document.getElementById('warrantyForm');
    form.reset();
    selectedWarrantyPhotos = [];
    updateWorkflowPhotoVisual('warranty', [], 0);
    renderEditAttachmentPreviews(null, 'warrantyEditAttachmentPreviews', []);
    ...
    if (claim) {
        ...
        if (claim.photos?.length) {
            renderEditAttachmentPreviews(claim, 'warrantyEditAttachmentPreviews', claim.photos.map(filename => ({ folder: 'warranty', filename, kind: 'photo' })));
        }
    } else if (pendingWarrantyTransfer?.sourceInspectionId) {
        const source = orders.find(order => order.id == pendingWarrantyTransfer.sourceInspectionId && isInspectionOrder(order));
        if (source) {
            document.getElementById('modalWarrantyTitle').textContent = 'Create Warranty from Inspection';
            document.getElementById('warrantyJobNumber').value = source.job_number || '';
            document.getElementById('warrantyReportedIssue').value = source.notes || '';
            document.getElementById('warrantyOrderType').value = '';
            document.getElementById('warrantyClaimType').value = 'Damage/Defect';
            document.getElementById('warrantyStatus').value = 'Working on it';
            document.getElementById('warrantyOriginalOrderId').value = '';
            document.getElementById('warrantyCustomerName').value = source.customer_name || '';
            document.getElementById('warrantyNotes').value = '';
            updateWorkflowPhotoVisual('warranty', null, source.photos?.length || 0);
            if (source.photos?.length) {
                renderEditAttachmentPreviews(source, 'warrantyEditAttachmentPreviews', source.photos.map(filename => ({ folder: 'panel_inspections', filename, kind: 'photo' })));
            }
        }
    }
    toggleWarrantyFields();
...
function createWarrantyFromInspection(inspectionId) {
    const source = orders.find(order => order.id == inspectionId && isInspectionOrder(order));
    if (!source || source.status !== 'Inspected - damage found') return;
    openWarrantyModal(null, source.id);
}

async function copyInspectionPhotosToWarranty(sourceInspection, jobNumber) {
    const sourcePhotos = Array.isArray(sourceInspection?.photos) ? sourceInspection.photos : [];
    if (!sourcePhotos.length) return [];

    const client = getSupabaseClient();
    const jobSafe = String(jobNumber || sourceInspection.job_number || 'inspection')
        .replace(/[^a-zA-Z0-9_-]/g, '_') || 'inspection';
    const copied = [];
    showUploadProgress(0, sourcePhotos.length);
    for (let idx = 0; idx < sourcePhotos.length; idx++) {
        showUploadProgress(idx, sourcePhotos.length);
        const sourceFilename = sourcePhotos[idx];
        const ext = String(sourceFilename).split('.').pop() || 'jpg';
        const targetFilename = `warranty_from_inspection_${jobSafe}_${Date.now()}_${idx}_${Math.floor(Math.random() * 1000)}.${ext}`;
        const fromPath = `panel_inspections/${sourceFilename}`;
        const toPath = `warranty/${targetFilename}`;
        const bucket = client.storage.from(SUPABASE_STORAGE_BUCKET);
        const copyResult = typeof bucket.copy === 'function'
            ? await bucket.copy(fromPath, toPath)
            : { error: new Error('Storage copy is not available') };
        if (copyResult.error) {
    ...
    showUploadProgress(sourcePhotos.length, sourcePhotos.length);
    return copied;
}

async function askDeleteTransferredInspection(sourceInspection) {
    if (!sourceInspection) return;
    const shouldDelete = confirm(`Warranty was created from Panel Inspection Job ${sourceInspection.job_number || '-'}. Delete the original Panel Inspection record now?`);
    if (!shouldDelete) return;

    const previousInspection = JSON.parse(JSON.stringify(sourceInspection));
    try {
        await deleteOrderRecord(sourceInspection.id);
        showToast('Panel Inspection deleted', 'You can restore it for a few seconds.', 'success', {
            label: 'Undo',
            handler: async () => {
                try {
                    await saveOrderRecord(previousInspection);
                    await syncDatabase();
                    showToast('Panel Inspection restored', 'The original record is back.');
                } catch (err) {
                    console.error('Inspection restore failed:', err);
                    showToast('Restore failed', 'Could not restore the inspection record.', 'error');
                }
            }
        });
        await syncDatabase();
    } catch (err) {
        console.error('Inspection delete failed:', err);
        showToast('Delete failed', 'The warranty was saved, but the inspection could not be deleted.', 'error');
    }
}
...
async function handleWarrantySubmit(e) {
    ...
    const id = editId || Date.now();
    const transferSource = !editId && pendingWarrantyTransfer?.sourceInspectionId
        ? orders.find(order => order.id == pendingWarrantyTransfer.sourceInspectionId && isInspectionOrder(order))
        : null;
    ...
    if (transferSource?.photos?.length) {
        try {
            const copiedPhotos = await copyInspectionPhotosToWarranty(transferSource, document.getElementById('warrantyJobNumber').value.trim());
            photos = [...photos, ...copiedPhotos];
            hideUploadProgress();
        } catch (err) {
            hideUploadProgress();
            console.error('Inspection photo transfer failed:', err);
            alert('Inspection photos could not be transferred. The warranty claim was not saved.');
            return;
        }
    }
    ...
    try {
        await saveOrderRecord(claim);
        const completedTransferSource = transferSource ? JSON.parse(JSON.stringify(transferSource)) : null;
        pendingWarrantyTransfer = null;
        closeAllModalsAfterSave();
        currentDashboardTab = 'warranty';
        await syncDatabase();
        if (completedTransferSource) {
            showToast('Warranty created', 'Panel Inspection information and photos were transferred.', 'success');
            await askDeleteTransferredInspection(completedTransferSource);
        }
    } catch (err) {
...
function openInspectionModal(orderId = null) {
    ...
    renderEditAttachmentPreviews(null, 'inspectionEditAttachmentPreviews', []);
    ...
    if (inspection) {
        ...
        if (inspection.photos?.length) {
            renderEditAttachmentPreviews(inspection, 'inspectionEditAttachmentPreviews', inspection.photos.map(filename => ({ folder: 'panel_inspections', filename, kind: 'photo' })));
        }
    }
    toggleInspectionFields();
...
function viewWarrantyPhotos(orderId, startFilename = null) {
    const claim = orders.find(order => order.id == orderId && isWarrantyOrder(order));
    ...
    let startIndex = 0;
    if (startFilename) startIndex = currentGalleryFiles.findIndex(f => f.filename === startFilename);
    if (startIndex === -1) startIndex = 0;
    openGalleryModal(startIndex);
}

function viewInspectionPhotos(orderId, startFilename = null) {
...
function viewStockPartPhoto(itemId) {
    const item = stockParts.find(part => part.id === itemId);
    ...
    currentGalleryFiles = [{
        ...
        filename: item.photo,
        itemId: item.id,
        storageFolder: 'stock_parts',
        title: `Part Photo - ${item.model || 'Stock Part'}`
    }];
    openGalleryModal(0);
}

async function hydrateStockPartPhoto(item) {
    const container = Array.from(document.querySelectorAll('.stock-part-photo-viewer'))
        .find(element => element.dataset.stockPartId === String(item.id));
    if (!container || !item?.photo) return;
    try {
        const url = await getStorageFileUrl('stock_parts', item.photo);
        container.innerHTML = `
            <button type="button" onclick="viewStockPartPhoto('${escapeHtml(item.id)}')" title="Open part photo">
                <img src="${escapeHtml(url)}" alt="${escapeHtml(item.model || 'Stock part photo')}">
            </button>`;
    } catch (err) {
        console.error('Failed to load stock part photo:', err);
        container.innerHTML = '<div class="text-muted">Photo preview unavailable.</div>';
    }
}
...
async function viewTrailerIssuePhotos(orderId, startFilename = null) {
    const trailer = orders.find(order => order.id == orderId && isTrailerOrder(order));
...
function previewFileToGalleryFile(file, order, index, total) {
    const galleryType = file.galleryType || (
        file.folder === 'osd' ? 'osd'
        : file.folder === 'clopay_issues' ? 'trailer_issue'
        : file.folder === 'shipped_out' ? 'shipped_out'
        : file.folder === 'customer_pickup' ? 'customer_pickup'
        : file.folder === 'warranty' ? 'warranty'
        : file.folder === 'panel_inspections' ? 'panel_inspection'
        : 'attachment'
    );
    const label = file.kind === 'pdf'
        ? (file.title || `PDF Document ${index + 1}`)
        : isTrailerOrder(order)
        ? `Issue Photo ${index + 1} of ${total}`
        : isOutboundOrder(order)
        ? `Shipped Freight Photo ${index + 1} of ${total}`
        : isPickupOrder(order)
        ? `Customer Pick Up Photo ${index + 1} of ${total}`
        : isWarrantyOrder(order)
        ? `Warranty Photo ${index + 1} of ${total}`
        : isInspectionOrder(order)
        ? `Inspection Photo ${index + 1} of ${total}`
...
async function openRecordAttachmentPreview(orderId, filename) {
    const order = orders.find(item => item.id == orderId);
    if (!order || !filename) return;

    currentGalleryOrder = order;
    const previews = getRecordPreviewFiles(order);
    currentGalleryFiles = previews.map((file, index) => previewFileToGalleryFile(file, order, index, previews.length));
    let startIndex = currentGalleryFiles.findIndex(file => file.filename === filename);
    if (startIndex === -1) startIndex = 0;
    openGalleryModal(startIndex);
}

async function openRecordPhotoPreview(orderId, filename) {
    const order = orders.find(item => item.id == orderId);
    if (!order || !filename) return;
    ...
    if (isReceivingOrder(order)) {
        ...
        await viewTrailerIssuePhotos(orderId, filename);
    } else if (isOutboundOrder(order)) {
        await viewOutboundPhotos(orderId, filename);
    } else if (isPickupOrder(order)) {
        await viewPickupPhotos(orderId, filename);
    } else if (isWarrantyOrder(order)) {
        viewWarrantyPhotos(orderId, filename);
    } else if (isInspectionOrder(order)) {
        viewInspectionPhotos(orderId, filename);
    }
}
...
async function loadGalleryFile(index) {
    ...
    const galleryRecordLabel = fileObj.type === 'stock_part'
        ? 'Stock Parts'
        : currentGalleryOrder
    ...
    const wrapper = document.getElementById('galleryMediaWrapper');
    wrapper.innerHTML = '<div class="gallery-loader"><i class="fa-solid fa-spinner fa-spin"></i><span>Loading file...</span></div>';
    const annotateButton = document.getElementById('btnAnnotateGalleryPhoto');
    updateGalleryDeleteButtonState(fileObj);
    ...
    try {
        ...
        const modalOverlay = document.getElementById('modalViewScan');
        annotateButton?.classList.toggle('hidden', !isImage);
        
        if (isPdf) {
            modalContent.classList.add('pdf-active');
            modalOverlay?.classList.add('pdf-document-active');
            ...
            iframe.src = fileUrl;
            iframe.id = 'galleryDocument';
            iframe.className = 'gallery-pdf-frame';
            iframe.style.setProperty('--zoom-scale', '1');
            iframe.style.setProperty('--rotate-angle', '0deg');
            iframe.style.setProperty('--pan-x', '0px');
            iframe.style.setProperty('--pan-y', '0px');
            ...
            modalOverlay?.classList.remove('pdf-document-active');
        ...
        wrapper.innerHTML = '<div class="gallery-error"><i class="fa-solid fa-triangle-exclamation"></i><p>File not found or access denied.</p></div>';
    }
...
function getGalleryPhotoDeleteTarget(fileObj) {
    if (!fileObj?.filename || !isImageFilename(fileObj.filename)) return null;

    if (fileObj.type === 'stock_part') {
        const item = stockParts.find(part => part.id === fileObj.itemId || part.photo === fileObj.filename);
        return item ? { kind: 'stock_part', item, field: 'photo' } : null;
    }

    const order = currentGalleryOrder;
    if (!order) return null;

    const arrayTarget = field => ({ kind: 'order_array', order, field });
    const targets = {
        packing_slip: arrayTarget('packing_slip_filenames'),
        pod: arrayTarget('invoice_filenames'),
        osd: arrayTarget('osd_photos'),
        shipped_out: arrayTarget('shipped_photos'),
        customer_pickup: arrayTarget('pickup_photos'),
        warranty: arrayTarget('photos'),
        panel_inspection: arrayTarget('photos'),
        trailer_issue: arrayTarget('issue_photos')
    };
    return targets[fileObj.type] || null;
}

function updateGalleryDeleteButtonState(fileObj = currentGalleryFiles[currentGalleryIndex]) {
    const button = document.getElementById('btnDeleteGalleryPhoto');
    if (!button) return;
    const canDelete = Boolean(getGalleryPhotoDeleteTarget(fileObj));
    button.classList.toggle('hidden', !canDelete);
    button.disabled = !canDelete;
}

function removeFilenameFromRecordList(record, field, filename) {
    const current = field === 'packing_slip_filenames'
        ? getPackingSlipFiles(record)
        : field === 'invoice_filenames'
        ? (record.invoice_filenames || (record.invoice_filename ? [record.invoice_filename] : []))
        : (Array.isArray(record[field]) ? record[field] : []);
    record[field] = current.filter(item => item !== filename);
    if (field === 'packing_slip_filenames') {
        record.packing_slip_filename = record.packing_slip_filenames[0] || null;
        if (Array.isArray(record.packing_slip_file_meta)) {
            record.packing_slip_file_meta = record.packing_slip_file_meta.filter(meta => meta?.filename !== filename);
        }
    }
    if (field === 'invoice_filenames') {
        record.invoice_filename = record.invoice_filenames[0] || null;
        if (Array.isArray(record.invoice_file_meta)) {
            record.invoice_file_meta = record.invoice_file_meta.filter(meta => meta?.filename !== filename);
        }
    }
}

function restoreFilenameToRecordList(record, field, filename, previousRecord) {
    record[field] = Array.isArray(previousRecord[field]) ? [...previousRecord[field]] : [];
    if (field === 'packing_slip_filenames') {
        record.packing_slip_filename = previousRecord.packing_slip_filename || record[field][0] || null;
        record.packing_slip_file_meta = Array.isArray(previousRecord.packing_slip_file_meta) ? [...previousRecord.packing_slip_file_meta] : [];
    }
    if (field === 'invoice_filenames') {
        record.invoice_filename = previousRecord.invoice_filename || record[field][0] || null;
        record.invoice_file_meta = Array.isArray(previousRecord.invoice_file_meta) ? [...previousRecord.invoice_file_meta] : [];
    }
    if (!record[field].includes(filename)) record[field].push(filename);
}

function rebuildCurrentGalleryFilesAfterPhotoChange(fileObj) {
    if (!fileObj) return;
    if (fileObj.type === 'stock_part') {
        const item = stockParts.find(part => part.id === fileObj.itemId || part.photo === fileObj.filename);
        currentGalleryFiles = item?.photo ? [{
            type: 'stock_part',
            filename: item.photo,
            itemId: item.id,
            storageFolder: 'stock_parts',
    ...
    if (!currentGalleryOrder) return;
    if (['packing_slip', 'pod', 'osd'].includes(fileObj.type)) {
        buildGallery(currentGalleryOrder);
        return;
    }
    if (fileObj.type === 'shipped_out') {
        currentGalleryFiles = (currentGalleryOrder.shipped_photos || []).map((filename, index) => ({
            type: 'shipped_out',
            filename,
            storageFolder: 'shipped_out',
            title: `Shipped Freight Photo ${index + 1} of ${currentGalleryOrder.shipped_photos.length}`
        }));
    } else if (fileObj.type === 'customer_pickup') {
        currentGalleryFiles = (currentGalleryOrder.pickup_photos || []).map((filename, index) => ({
            type: 'customer_pickup',
            filename,
            storageFolder: 'customer_pickup',
            title: `Customer Pick Up Photo ${index + 1} of ${currentGalleryOrder.pickup_photos.length}`
        }));
    } else if (fileObj.type === 'warranty') {
        currentGalleryFiles = (currentGalleryOrder.photos || []).map((filename, index) => ({
            type: 'warranty',
            filename,
            storageFolder: 'warranty',
            title: `Warranty Photo ${index + 1} of ${currentGalleryOrder.photos.length}`
        }));
    } else if (fileObj.type === 'panel_inspection') {
        currentGalleryFiles = (currentGalleryOrder.photos || []).map((filename, index) => ({
            type: 'panel_inspection',
            filename,
            storageFolder: 'panel_inspections',
            title: `Inspection Photo ${index + 1} of ${currentGalleryOrder.photos.length}`
        }));
    } else if (fileObj.type === 'trailer_issue') {
        currentGalleryFiles = (currentGalleryOrder.issue_photos || []).map((filename, index) => ({
            type: 'trailer_issue',
            filename,
            storageFolder: 'clopay_issues',
            title: `Issue Photo ${index + 1} of ${currentGalleryOrder.issue_photos.length}`
        }));
    }
}

function refreshUiAfterGalleryPhotoChange(target) {
    if (target.kind === 'stock_part') {
        renderStockTable();
        if (!document.getElementById('modalStockDetail')?.classList.contains('hidden')) {
            openStockDetail('parts', target.item.id);
        }
        return;
    }
    renderDashboard();
    const viewerOpen = !document.getElementById('modalViewScan')?.classList.contains('hidden');
    if (!viewerOpen && !document.getElementById('modalViewOrderDetails')?.classList.contains('hidden')) {
        viewOrderDetails(target.order.id);
    }
}

async function persistGalleryPhotoTarget(target) {
    if (target.kind === 'stock_part') {
        await saveStockData('parts');
    } else {
        target.order.updated_date = getLocalDateTimeString();
        const { error } = await getSupabaseClient()
            .from(SUPABASE_ORDERS_TABLE)
            .upsert({ id: Number(target.order.id), data: target.order }, { onConflict: 'id' });
        if (error) throw error;
    }
}

async function deleteCurrentGalleryPhoto() {
    const fileObj = currentGalleryFiles[currentGalleryIndex];
    const target = getGalleryPhotoDeleteTarget(fileObj);
    if (!fileObj || !target) return;
    if (!confirm('Delete this photo? You will be able to undo this for a few seconds.')) return;

    const deleteButton = document.getElementById('btnDeleteGalleryPhoto');
    const originalButtonHtml = deleteButton?.innerHTML || '';
    if (deleteButton) {
        deleteButton.disabled = true;
        deleteButton.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i><span>Deleting</span>';
    }

    const path = `${fileObj.storageFolder}/${fileObj.filename}`;
    const previousOrder = target.kind === 'order_array' ? JSON.parse(JSON.stringify(target.order)) : null;
    const previousPhoto = target.kind === 'stock_part' ? target.item.photo : null;

    try {
        const bucket = getSupabaseClient().storage.from(SUPABASE_STORAGE_BUCKET);
        const { data: backupBlob, error: downloadError } = await bucket.download(path);
        if (downloadError) throw downloadError;

        if (target.kind === 'stock_part') {
            target.item.photo = '';
        } else {
            removeFilenameFromRecordList(target.order, target.field, fileObj.filename);
        }

        await persistGalleryPhotoTarget(target);
        const { error: removeError } = await bucket.remove([path]);
        if (removeError) throw removeError;

        rebuildCurrentGalleryFilesAfterPhotoChange(fileObj);

        if (currentGalleryFiles.length) {
            const nextIndex = Math.min(currentGalleryIndex, currentGalleryFiles.length - 1);
            await loadGalleryFile(nextIndex);
        } else {
            closeViewerModal();
        }
        refreshUiAfterGalleryPhotoChange(target);

        showToast('Photo deleted', 'You can restore it for a few seconds.', 'success', {
            label: 'Undo',
            handler: async () => {
                try {
                    await bucket.upload(path, backupBlob, { upsert: true, contentType: backupBlob.type || 'application/octet-stream' });
                    if (target.kind === 'stock_part') {
                        target.item.photo = previousPhoto || fileObj.filename;
                    } else {
                        restoreFilenameToRecordList(target.order, target.field, fileObj.filename, previousOrder);
                    }
                    await persistGalleryPhotoTarget(target);
                    rebuildCurrentGalleryFilesAfterPhotoChange(fileObj);
                    refreshUiAfterGalleryPhotoChange(target);
                    if (!document.getElementById('modalViewScan')?.classList.contains('hidden')) {
                        const restoredIndex = currentGalleryFiles.findIndex(item => item.filename === fileObj.filename);
                        await loadGalleryFile(restoredIndex === -1 ? 0 : restoredIndex);
                    }
                    showToast('Photo restored', 'The photo is back.');
                } catch (err) {
                    console.error('Photo restore failed:', err);
                    showToast('Restore failed', 'Could not restore the photo. Check connection.', 'error');
                }
            }
        });
    } catch (err) {
        ...
        if (target.kind === 'stock_part') {
            target.item.photo = previousPhoto;
        } else if (previousOrder) {
            Object.assign(target.order, previousOrder);
        }
        try {
            await persistGalleryPhotoTarget(target);
        } catch (rollbackErr) {
            console.error('Photo delete rollback failed:', rollbackErr);
        }
        showToast('Delete failed', 'Could not delete the photo. Check connection.', 'error');
    } finally {
        if (deleteButton) {
            deleteButton.innerHTML = originalButtonHtml;
            updateGalleryDeleteButtonState(currentGalleryFiles[currentGalleryIndex]);
        }
    }
}
...
function getPdfReportData(order) {
    ...
    if (isOutboundOrder(order)) {
        ...
        fields: [['Customer', order.customer], ['Job Number', order.job_number], ['Status', getOutboundStatus(order)], ['Courier', order.carrier], ['Ship To Address', order.ship_to_address], ['Payment Type', order.payment_type], ['Shipped Date', getOutboundStatus(order) === 'Shipped' ? order.shipped_date : ''], ['Notes', order.notes]],
            attachments: [],
...
async function generateOrderPdfReport(orderId) {
    ...
    const statusColor = status => {
        if (['Received', 'Dispatched', 'Approved', 'Inspected - good to go', 'Shipped Out', 'Shipped', 'Picked Up'].includes(status)) return [22, 163, 74];
        if (['To be Shipped', 'Working on it'].includes(status)) return [245, 158, 11];
        if (status === 'To be Claimed') return [17, 24, 39];
        if (['Denied', 'Inspected - damage found'].includes(status)) return [220, 38, 38];
        if (['Request Submitted', 'Arrived'].includes(status)) return [37, 99, 235];
        return [107, 114, 128];
    };
    const drawPdfLogo = () => {
        const boxWidth = 66;
...
function getGalleryZoomTarget() {
    return document.getElementById('galleryImage') || document.getElementById('galleryDocument');
}
...
function applyGalleryTransform(target) {
    if (!target) return;
    target.style.setProperty('--zoom-scale', String(zoomScale));
    target.style.setProperty('--pan-x', `${panOffset.x}px`);
    target.style.setProperty('--pan-y', `${panOffset.y}px`);
    target.style.setProperty('--rotate-angle', `${rotateAngle}deg`);
    document.getElementById('galleryZoomLabel').textContent = `${Math.round(zoomScale * 100)}%`;
}

function setupGalleryTouchZoom(target) {
    const viewport = document.getElementById('galleryViewport');
    ...
    viewport.ontouchmove = event => {
        if (event.touches.length === 2 && pinchStartDistance) {
            ...
            zoomScale = Math.min(4, Math.max(0.75, nextScale));
            if (zoomScale <= 1) panOffset = { x: 0, y: 0 };
            applyGalleryTransform(target);
        } else if (event.touches.length === 1 && isPanning && zoomScale > 1) {
            event.preventDefault();
            panOffset = {
                x: event.touches[0].clientX - touchPanStart.x,
                y: event.touches[0].clientY - touchPanStart.y
            };
            applyGalleryTransform(target);
        }
    };
...
function galleryZoomIn() {
    const target = getGalleryZoomTarget();
    if (!target) return;
...
function viewOrderDetails(orderId) {
    ...
    if (isWarrantyOrder(order) || isInspectionOrder(order)) {
        ...
        const photoAction = `${photos.length
            ? `<button class="btn btn-secondary" onclick="${warranty ? 'viewWarrantyPhotos' : 'viewInspectionPhotos'}('${order.id}');"><i class="fa-solid fa-camera"></i> View Photos (${photos.length})</button>`
    ...
    if (isTrailerOrder(order)) {
        ...
        const documentAction = documents.length > 0
            ? `<button class="btn btn-secondary" onclick="viewTrailerDocuments('${order.id}');"><i class="fa-solid fa-paperclip"></i> View Documents (${documents.length})</button>`
        ...
        const issuePhotoAction = `${issuePhotos.length > 0
            ? `<button class="btn btn-accent" onclick="viewTrailerIssuePhotos('${order.id}');" style="background: var(--status-error); border-color: var(--status-error);"><i class="fa-solid fa-camera"></i> View Issue Photos (${issuePhotos.length})</button>`
    ...
    if (isOutboundOrder(order) || isPickupOrder(order)) {
        ...
        const photoAction = `${photos.length > 0
            ? `<button class="btn btn-secondary" onclick="${isPickup ? 'viewPickupPhotos' : 'viewOutboundPhotos'}('${order.id}');"><i class="fa-solid fa-camera"></i> View Photos (${photos.length})</button>`
            : '<span class="text-muted">No photos attached</span>'}
        ...
        ` : `${timelineHtml}
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem; font-size: 0.95rem; line-height: 1.5;">
                <div>
                    ...
                    <p><strong>Status:</strong> <span class="badge ${workflowStatusClass(getOutboundStatus(order), order)}">${escapeHtml(getOutboundStatus(order))}</span></p>
                ...
                <div>
                    ...
                    <p><strong>Shipped Date:</strong> ${getOutboundStatus(order) === 'Shipped' ? formatDateTimeDisplay(order.shipped_date) : '-'}</p>
    ...
    const packingSlipFiles = getPackingSlipFiles(order);
    if (packingSlipFiles.length > 0) {
        attachmentsHtml += `<button class="btn btn-secondary" onclick="viewFileAttachment('${order.id}', 'packing_slip');" style="margin-right: 0.5rem; margin-bottom: 0.5rem;"><i class="fa-solid fa-file-invoice"></i> Attached Files (${packingSlipFiles.length})</button>`;
    }
    const podFiles = order.invoice_filenames || (order.invoice_filename ? [order.invoice_filename] : []);
    if (podFiles.length > 0) {
        attachmentsHtml += `<button class="btn btn-secondary" onclick="viewFileAttachment('${order.id}', 'pod');" style="margin-right: 0.5rem; margin-bottom: 0.5rem;"><i class="fa-solid fa-file-signature"></i> POD (${podFiles.length})</button>`;
    }
    if (order.osd_photos && order.osd_photos.length > 0) {
        attachmentsHtml += `<button class="btn btn-accent" onclick="viewOsdPhotos('${order.id}');" style="margin-right: 0.5rem; margin-bottom: 0.5rem; background: var(--status-error); border-color: var(--status-error);"><i class="fa-solid fa-camera"></i> Damage Photos (${order.osd_photos.length})</button>`;
    }
...
function setRecordEditHandlers(order) {
    const handler = () => {
        ...
        else if (isReceivingOrder(order) && order.status === 'Received') openReceiveModal(order.id);
...
function renderRecordOverview(order) {
    if (isReceivingOrder(order)) {
        ...
        return renderDetailSection('Overview', 'fa-circle-info', renderInfoGrid([
            ...
            ['ETA', etaValue],
            ['ETA Arrival', renderReceivedEtaBadge(order) || '-'],
    ...
    if (isOutboundOrder(order)) {
        ...
        return renderDetailSection('Overview', 'fa-circle-info', renderInfoGrid([
            ...
            ['Status', `<span class="badge ${workflowStatusClass(getOutboundStatus(order), order)}">${escapeHtml(getOutboundStatus(order))}</span>`],
            ['Courier', escapeHtml(carrierOptionLabel(order.carrier || 'LTL / Other'))],
            ['Tracking Number', trackingValue],
            ['Payment Type', escapeHtml(order.payment_type || '-')],
            ['Shipped Date', getOutboundStatus(order) === 'Shipped' ? formatDateTimeDisplay(order.shipped_date) : '-'],
...
function renderRecordNotes(order) {
    if (isReceivingOrder(order)) {
        return renderDetailSection('Description', 'fa-align-left', renderTextBlock(order.item_description, 'No description logged.')) +
            (order.has_issue ? '' : renderDetailSection('Notes', 'fa-note-sticky', renderTextBlock(order.notes, 'No receiving notes logged.')));
    }
    ...
    if (isWarrantyOrder(order)) {
        const warrantyNotes = (order.warranty_notes || '').trim();
        return renderDetailSection('Reported Issue', 'fa-triangle-exclamation', renderTextBlock(order.reported_issue, 'No issue logged.'), order.status === 'Denied' ? '' : '') +
            (warrantyNotes ? renderDetailSection('Warranty Notes', 'fa-note-sticky', renderTextBlock(warrantyNotes, '')) : '');
    }
    if (isInspectionOrder(order) && order.status === 'Inspected - damage found') return '';
...
function getTrackingUrl(carrier, trackingNumber) {
    ...
    switch (carrier) {
        ...
        case 'ABF':
            return `https://view.arcb.com/nlo/tools/tracking/${encodedNum}`;
        case 'TST-express':
...
function setupUploads() {
    ensureEditAttachmentPreview('orderEditAttachmentPreviews', 'orderFileSelectionStatus');
    ensureEditAttachmentPreview('receivePodEditAttachmentPreviews', 'fileSelectionStatus');
    ensureEditAttachmentPreview('receiveOsdEditAttachmentPreviews', 'osdFileSelectionStatus');
    ensureEditAttachmentPreview('outboundEditAttachmentPreviews', 'outboundPhotoSelectionStatus');
    ensureEditAttachmentPreview('pickupEditAttachmentPreviews', 'pickupPhotoSelectionStatus');
    ensureEditAttachmentPreview('trailerDocsEditAttachmentPreviews', 'trailerDocsSelectionStatus');
    ensureEditAttachmentPreview('trailerIssueEditAttachmentPreviews', 'trailerIssuePhotoSelectionStatus');
    ensureEditAttachmentPreview('warrantyEditAttachmentPreviews', 'warrantyPhotoSelectionStatus');
    ensureEditAttachmentPreview('inspectionEditAttachmentPreviews', 'inspectionPhotoSelectionStatus');
...
function setupTrailerDocumentUpload() {
    ...
    const chooseFiles = files => {
        ...
        renderSelectedFilePreviews('trailerDocsEditAttachmentPreviews', valid);
    };
...
function setupMultipleUpload(zoneId, inputId, statusId, textId, clearId, setFilesCallback, isImageOnly = false) {
    ...
    clearBtn.addEventListener('click', (e) => {
        ...
        textEl.textContent = 'No files selected';
        const previewId = {
            orderFileSelectionStatus: 'orderEditAttachmentPreviews',
            fileSelectionStatus: 'receivePodEditAttachmentPreviews',
            osdFileSelectionStatus: 'receiveOsdEditAttachmentPreviews',
            outboundPhotoSelectionStatus: 'outboundEditAttachmentPreviews',
            pickupPhotoSelectionStatus: 'pickupEditAttachmentPreviews',
            trailerIssuePhotoSelectionStatus: 'trailerIssueEditAttachmentPreviews'
        }[statusId];
        if (previewId) renderEditAttachmentPreviews(null, previewId, []);
    ...
    async function handleMultipleFilesSelection(files) {
        ...
        if (previewId) renderSelectedFilePreviews(previewId, preparedFiles);
...
function openAdminSettingsModal() {
    ...
    currentAdminRecordFilter = 'all';
    document.querySelectorAll('.admin-record-tab').forEach(tab => {
        tab.classList.toggle('active', tab.dataset.adminRecordFilter === 'all');
    });
...
async function handleAdminSettingsSubmit(e) {
    e.preventDefault();
    if (e.submitter && e.submitter.id !== 'btnSaveAdminSettings') return;
        ...
        if (blob) {
            ...
            if (currentAnnotationTarget === 'saved' && currentAnnotationSavedMeta) {
                uploadStorageFile(currentAnnotationSavedMeta.folder, currentAnnotationSavedMeta.filename, annotatedFile)
                    .then(async () => {
                        const savedMeta = currentAnnotationSavedMeta;
                        const annotatedOrder = orders.find(item => item.id == savedMeta.orderId);
                        if (annotatedOrder) {
                            annotatedOrder.photo_annotations = {
                                ...(annotatedOrder.photo_annotations || {}),
                                [`${savedMeta.folder}/${savedMeta.filename}`]: getLocalDateTimeString()
                            };
                            try {
                                await saveOrderRecord(annotatedOrder);
                            } catch (err) {
                                console.warn('Photo annotation marker was not saved:', err);
                            }
                        }
                        ...
                        if (annotatedOrder && !document.getElementById('modalViewOrderDetails')?.classList.contains('hidden')) {
                            renderDetailPhotoPreviewStrip(annotatedOrder);
                        }
...
function renderAdminShipmentsList() {
    ...
    const filtered = orders.filter(order => {
        const recordType = isWarrantyOrder(order) ? 'warranty'
            : isInspectionOrder(order) ? 'inspections'
            : isTrailerOrder(order) ? 'trailers'
            : isPickupOrder(order) ? 'pickup'
            : isOutboundOrder(order) ? 'outbound'
            : 'receiving';
        if (currentAdminRecordFilter !== 'all' && recordType !== currentAdminRecordFilter) return false;
    ...
    if (filtered.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; color: var(--text-secondary); padding: 1rem;">No matching records.</td></tr>`;
        return;
    }
    ...
    filtered.forEach(order => {
        ...
        const status = (trailer || warranty || inspection) ? order.status : (pickup ? 'Picked Up' : (outbound ? getOutboundStatus(order) : order.status));
...
function getStockSearchPreviewValues(item) {
    const values = [];
    const add = (label, value) => {
        const text = String(value ?? '').trim();
        if (text && text !== '-') values.push(`${label}: ${text}`);
    };
    add('In stock', item.qty ?? 0);
    add('On order', item.on_order ?? 0);
    add('Re-order', item.re_order ?? item.low_stock_threshold);
    add('Size', item.size);
    add('Type', item.type);
...
function getAllStockSearchRecords() {
    const build = (items, type, group) => items.map(item => {
        ...
        return {
            ...
            previewValues: getStockSearchPreviewValues(item)
        };
    });
    return [
...
function renderStockLoadingState() {
    ...
    if (tableBody) tableBody.innerHTML = `
        <tr>
            <td colspan="5">
                <div class="apple-skeleton" aria-label="Loading stock data">
                    <span class="apple-skeleton-row"></span>
                    <span class="apple-skeleton-row"></span>
                    <span class="apple-skeleton-row"></span>
                    <span class="apple-skeleton-row"></span>
                </div>
...
function syncStockFilters(items, isDoors, isOperators, isParts) {
    ...
    const filterRow = document.getElementById('stockFilterRow');
    if (!sizeFilter || !typeFilter || !filterBar) return;
    const showFilters = isDoors || isOperators || isParts;
...
function placeStockFilterBar(isDoors, isOperators, isParts) {
    ...
    if (!filterBar) return;
    const filterRow = document.getElementById('stockFilterRow');
    if (filterRow && filterBar.parentElement !== filterRow) filterRow.appendChild(filterBar);
    filterRow?.classList.toggle('hidden', !(isDoors || isOperators || isParts));
}
...
function renderStockTable() {
    ...
    const stockFilterRow = document.getElementById('stockFilterRow');
    ...
    if (currentStockSubTab === 'analytics') {
        ...
        stockFilterRow?.classList.add('hidden');
    ...
    renderStockTableHeader(currentStockSubTab);
    ...
    if (items.length === 0) {
        emptyState.classList.remove('hidden');
        const stockFiltered = Boolean(searchVal || sizeFilter !== 'all' || typeFilter !== 'all');
        const stockEmptyIcon = emptyState.querySelector('.empty-state-icon i');
        const stockEmptyTitle = emptyState.querySelector('h3');
        const stockEmptyText = emptyState.querySelector('p');
        if (stockEmptyIcon) stockEmptyIcon.className = `fa-solid ${stockFiltered ? 'fa-filter-circle-xmark' : 'fa-box-open'}`;
        if (stockEmptyTitle) stockEmptyTitle.textContent = stockFiltered ? 'No matching stock' : 'No stock items yet';
        if (stockEmptyText) stockEmptyText.textContent = stockFiltered ? 'Clear search or choose a different stock filter.' : 'Add stock items from the inventory setup area to start tracking inventory.';
    ...
    items.forEach(item => {
        ...
        if (isDoors) {
            ...
            const photoButton = item.photo
                ? `<button type="button" class="btn-scan attached photo-preview-trigger" data-preview-folder="stock_parts" data-preview-filename="${escapeHtml(item.photo)}" onclick="viewStockPartPhoto('${escapeHtml(item.id)}')" title="Open part photo"><i class="fa-solid fa-image"></i> Photo</button>`
                : '<span class="text-muted"><i class="fa-solid fa-minus"></i></span>';
            tr.innerHTML = `
                <td class="po-col" data-column="reference">
        ...
        appendConfiguredStockRow(tableBody, tr);
    });
}
...
function openStockDetail(type, itemId) {
    ...
    const partPhotoHTML = (type === 'parts' && item.photo) ? `
        <section class="record-detail-section">
            <h4 class="record-section-title"><i class="fa-solid fa-image"></i>Photo</h4>
            <div class="stock-part-photo-viewer" data-stock-part-id="${escapeHtml(item.id)}" data-stock-part-photo="${escapeHtml(item.photo)}">
                <button type="button" onclick="viewStockPartPhoto('${escapeHtml(item.id)}')" title="Open part photo">
                    <div class="text-muted" style="padding:1rem;">Loading photo...</div>
                </button>
            </div>
        </section>` : '';
    ...
    const orderControlsHTML = type === 'nonstandard' ? '' : `
                <div class="stock-qty-row stock-qty-row-action">
                    <div class="stock-qty-copy">
                        <span class="stock-qty-row-label">On Order</span>
                        <small>Receive into stock</small>
                    </div>
    ...
    const soldHTML = type === 'parts' ? '' : `
                <div class="stock-qty-row stock-qty-row-readonly">
                    <div class="stock-qty-copy">
                        <span class="stock-qty-row-label">Sold</span>
                        <small>${type === 'rails' ? 'Via operators' : 'Via notes'}</small>
                    </div>
            ...
            <div class="stock-quantities-grid">
                ...
                <div class="stock-qty-row stock-qty-row-main">
                    ...
                    ${type === 'operators' && (item.rail_type && item.rail_type !== 'None' || (Array.isArray(item.extra_types) && item.extra_types.length > 0)) ? `
                        <div class="stock-sync-accessories">
                            <label class="stock-sync-toggle" title="Sync changes with Rails & Extras">
                                <input type="checkbox" id="syncAccessoriesCheckbox_${escapeHtml(item.id)}" onchange="toggleSyncAccessories('${escapeHtml(item.id)}')">
                                Sync with Rails &amp; Extras
                            </label>
                            <div id="syncAccessoriesOptions_${escapeHtml(item.id)}" class="stock-sync-options hidden">
                                <!-- Rail size selector -->
    ...
    body.innerHTML = `<div class="record-detail-shell stock-detail-shell">${heroHTML}${infoHTML}${partPhotoHTML}${qtyHTML}${notesHTML}</div>`;
    if (type === 'parts' && item.photo) hydrateStockPartPhoto(item);
...
function buildStockNotesHTML(item) {
            ...
            <div class="stock-note-item ${note.type === 'Install' ? 'install' : 'supply'}" data-note-id="${escapeHtml(note.id)}">
                <div class="stock-note-header">
                    ...
                    <div class="stock-note-actions">
                        ${note.qty ? `<button type="button" class="stock-note-action stock-note-return-btn" onclick="returnStockNoteToStock('${escapeHtml(item.id)}', '${escapeHtml(note.id)}')" title="Return this sold quantity back to stock"><i class="fa-solid fa-rotate-left"></i><span>Return</span></button>` : ''}
                        <button type="button" class="stock-note-action stock-note-icon-action" onclick="editStockNote('${escapeHtml(item.id)}', '${escapeHtml(note.id)}')" title="Edit note"><i class="fa-solid fa-pen"></i></button>
                        <button type="button" class="stock-note-action stock-note-icon-action danger" onclick="deleteStockNote('${escapeHtml(item.id)}', '${escapeHtml(note.id)}')" title="Delete note"><i class="fa-solid fa-trash-can"></i></button>
                    </div>
                </div>
    ...
    return `
        <section class="record-detail-section" style="margin-top:1.25rem;">
            ...
            <div class="stock-note-submit-row">
...
function addStockReturnAdjustment(adjustments, item, type, amount) {
    const qty = Math.max(0, Number(amount) || 0);
    if (!item || !type || qty <= 0) return;
    const existing = adjustments.find(entry => entry.item === item && entry.type === type);
    if (existing) existing.amount += qty;
    else adjustments.push({ item, type, amount: qty, previousQty: Number(item.qty || 0) });
}

function collectStockNoteReturnAdjustments(note, baseItem, adjustments) {
    if (!note) return;
    if (note.rail_id) {
        const rail = stockRails.find(r => r.id === note.rail_id);
        if (rail !== baseItem) addStockReturnAdjustment(adjustments, rail, 'rails', note.rail_qty || note.qty);
    }
    if (Array.isArray(note.extras)) {
        note.extras.forEach(ext => {
            const extra = stockExtras.find(e => e.id === ext.id);
            if (extra !== baseItem) addStockReturnAdjustment(adjustments, extra, 'extras', ext.qty || note.qty);
        });
    } else if (note.extra_id) {
        const extra = stockExtras.find(e => e.id === note.extra_id);
        if (extra !== baseItem) addStockReturnAdjustment(adjustments, extra, 'extras', note.extra_qty || note.qty);
    }
    if (note.op_id) {
        const operator = stockOperators.find(o => o.id === note.op_id);
        if (operator !== baseItem) addStockReturnAdjustment(adjustments, operator, 'operators', note.op_qty || note.qty);
    }
    if (note.door_id) {
        const doorMatch = findStockDoorLikeItem(note.door_id);
        if (doorMatch.item && doorMatch.item !== baseItem) {
            addStockReturnAdjustment(adjustments, doorMatch.item, doorMatch.type, note.door_qty || note.qty);
        }
    }
}

function findLinkedStockNote(item, type, note) {
    if (!note?.linked_note_id) return { linkedItem: null, linkedItemType: '', linkedNote: null };
    let linkedItem = null;
    let linkedItemType = '';
    ...
    const linkedNote = linkedItem && Array.isArray(linkedItem.notes)
        ? linkedItem.notes.find(n => n.id === note.linked_note_id)
        : null;
    return { linkedItem, linkedItemType, linkedNote };
}

function refreshStockNotesPanel(itemId) {
    const { item } = findStockItem(itemId);
    const qtyInput = document.getElementById(`stockQtyInput_${itemId}`);
    if (qtyInput && item) qtyInput.value = item.qty ?? 0;
    const soldQty = Array.isArray(item?.notes) ? item.notes.reduce((sum, n) => sum + (Number(n.qty) || 0), 0) : 0;
    const soldEl = document.getElementById(`stockSoldDisplay_${itemId}`);
    if (soldEl) {
        soldEl.innerHTML = `<span class="stock-sold-num ${soldQty > 0 ? 'has-sold' : ''}">${soldQty}</span><span class="stock-qty-hint">via notes</span>`;
    }
...
async function returnStockNoteToStock(itemId, noteId) {
    const { item, type } = findStockItem(itemId);
    if (!item || !Array.isArray(item.notes)) return;
    if (!['doors', 'operators', 'nonstandard'].includes(type)) return;
    const note = item.notes.find(n => n.id === noteId);
    if (!note) return;
    const noteQty = Math.max(0, Number(note.qty) || 0);
    if (noteQty <= 0) {
        showToast('Nothing to return', 'This note does not have a quantity to return.', 'info');
        return;
    }
    const label = [item.model, item.size].filter(Boolean).join(' ') || item.id;
    if (!confirm(`Return ${noteQty} unit(s) from this sold note back to stock?\n\n${label}`)) return;

    const { linkedItem, linkedItemType, linkedNote } = findLinkedStockNote(item, type, note);
    const originalNotes = [...item.notes];
    const originalLinkedNotes = linkedItem && Array.isArray(linkedItem.notes) ? [...linkedItem.notes] : null;
    const adjustments = [];
    addStockReturnAdjustment(adjustments, item, type, noteQty);
    collectStockNoteReturnAdjustments(note, item, adjustments);
    if (linkedNote) collectStockNoteReturnAdjustments(linkedNote, item, adjustments);

    adjustments.forEach(entry => {
        entry.item.qty = Number(entry.item.qty || 0) + entry.amount;
    });
    item.notes = item.notes.filter(n => n.id !== noteId);
    if (linkedItem && Array.isArray(linkedItem.notes)) {
        linkedItem.notes = linkedItem.notes.filter(n => n.id !== note.linked_note_id);
    }

    const typesToSave = [...new Set([
        type,
        ...adjustments.map(entry => entry.type),
        linkedItem ? linkedItemType : ''
    ].filter(Boolean))];

    try {
        for (const saveType of typesToSave) {
            await saveStockData(saveType);
        }
        const returnedTotal = adjustments.reduce((sum, entry) => sum + entry.amount, 0);
        refreshStockNotesPanel(itemId);
        renderStockTable();
        if (!document.getElementById('modalNonStandardDoors')?.classList.contains('hidden')) {
            renderNonStandardDoorsList();
        }
        captureOpenModalStates();
        showToast('Returned to stock', `${returnedTotal} unit${returnedTotal === 1 ? '' : 's'} returned and the sold note was removed.`, 'success');
    } catch (err) {
        adjustments.forEach(entry => {
            entry.item.qty = entry.previousQty;
        });
        item.notes = originalNotes;
        if (linkedItem && originalLinkedNotes) {
            linkedItem.notes = originalLinkedNotes;
        }
        showToast('Return failed', 'Could not return this note to stock. Check connection.', 'error');
    }
}
...
async function deleteStockNote(itemId, noteId) {
    ...
    try {
        ...
        showToast('Note deleted', 'You can restore it for a few seconds.', 'success', {
            label: 'Undo',
            handler: async () => {
                item.notes = originalNotes;
                if (linkedItem) linkedItem.notes = originalLinkedNotes;
                try {
                    await saveStockData(type);
                    ...
                    captureOpenModalStates();
                    showToast('Note restored', 'The deleted note is back.');
                } catch (err) {
                    showToast('Restore failed', 'Could not restore note. Check connection.', 'error');
                }
            }
        });
...
function editStockNote(itemId, noteId) {
    ...
    noteEl.innerHTML = `
        <div class="stock-note-edit-panel">
            <div class="stock-note-form-head">
                <div>
                    <span class="record-drawer-eyebrow">Edit stock note</span>
                    <strong>Update sale or inventory movement</strong>
                </div>
            </div>
            <div class="stock-note-form-grid stock-note-edit-grid">
                <label class="stock-note-field stock-note-field-small">
                    <span>Type</span>
                ...
                <label class="stock-note-field stock-note-field-wide">
                    <span>Details</span>
                    <input type="text" id="editNoteText_${escapeHtml(noteId)}" value="${escapeHtml(note.text || '')}" placeholder="Customer, job, sales note, or reason...">
                </label>
                <label class="stock-note-field stock-note-field-qty">
                    <span>QTY</span>
            ...
            <div class="stock-note-edit-actions">
                <button type="button" class="btn btn-secondary stock-note-edit-btn" onclick="cancelEditNote('${escapeHtml(itemId)}', '${escapeHtml(noteId)}')">Cancel</button>
                <button type="button" class="btn btn-accent stock-note-edit-btn" onclick="saveEditNote('${escapeHtml(itemId)}', '${escapeHtml(noteId)}')"><i class="fa-solid fa-check"></i> Save</button>
            </div>
        </div>`;
        ...
        <div class="nonstandard-list">
            ${items.length ? items.map(item => `
                <div class="nonstandard-card ${item.pinned ? 'pinned' : ''}">
                    ...
                    <button type="button" class="btn btn-secondary btn-icon nonstandard-action-edit" onclick="editNonStandardDoor('${escapeHtml(item.id)}')" title="Edit"><i class="fa-solid fa-pen"></i></button>
                    <button type="button" class="btn btn-secondary btn-icon nonstandard-action-delete" onclick="deleteNonStandardDoor('${escapeHtml(item.id)}')" title="Delete" style="color:var(--status-error);"><i class="fa-solid fa-trash-can"></i></button>
...
function renderStockAnalytics(containerId = 'stockAnalyticsView') {
    const container = document.getElementById(containerId);
    if (!container) return;
    ...
    let totalPartsQty = stockParts.reduce((sum, i) => sum + (i.qty || 0), 0);
    let totalRailsQty = stockRails.reduce((sum, i) => sum + (i.qty || 0), 0);
    let totalExtrasQty = stockExtras.reduce((sum, i) => sum + (i.qty || 0), 0);
    let totalCurrentStock = totalDoorsQty + totalOpsQty + totalPartsQty + totalRailsQty + totalExtrasQty;
    ...
    let partsLowStock = stockParts.filter(i => i.low_stock_threshold !== undefined && i.low_stock_threshold !== null && (i.qty || 0) <= i.low_stock_threshold);
    ...
    if (searchVal) {
        ...
        partsLowStock = partsLowStock.filter(item => [item.model, item.type, item.description].some(v => v && String(v).toLowerCase().includes(searchVal)));
    ...
    let lowStockCount = doorsLowStock.length + opsLowStock.length + partsLowStock.length + railsLowStock.length + extrasLowStock.length;
    ...
    let html = `
        <div class="analytics-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1.25rem; margin-bottom: 2rem;">
            ...
            <!-- CARD 5: CURRENT STOCK PARTS LEVEL -->
            <div class="analytics-card card-stock-parts" style="background: linear-gradient(135deg, rgba(245, 158, 11, 0.1) 0%, rgba(180, 83, 9, 0.02) 100%); border: 1.5px solid rgba(245, 158, 11, 0.15); border-radius: 12px; padding: 1.25rem; display: flex; align-items: center; gap: 1rem; position: relative; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
                <div class="analytics-card-icon" style="background: rgba(245, 158, 11, 0.15); color: #b45309; width: 48px; height: 48px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 1.3rem;">
                    <i class="fa-solid fa-screwdriver-wrench"></i>
                </div>
                <div>
                    <span class="analytics-card-label" style="display: block; font-size: 0.76rem; text-transform: uppercase; font-weight: 700; color: var(--text-secondary); letter-spacing: 0.04em;">Stock Parts Level</span>
                    <strong class="analytics-card-val" style="display: block; font-size: 1.75rem; font-weight: 800; color: var(--text-primary); margin-top: 0.25rem;">${totalPartsQty}</strong>
                </div>
            </div>
        ...
        <div class="analytics-sections" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(360px, 1fr)); gap: 1.5rem;">
            ...
            <div class="analytics-section-card" style="border: 1px solid var(--border-color); border-radius: 12px; background: var(--bg-card); padding: 1.25rem; display:flex; flex-direction:column; gap:1.25rem;">
                ...
                <!-- PARTS LOW STOCK -->
                <div class="analytics-bordered-block">
                    <h4 style="margin: 0 0 0.75rem 0; font-size: 0.82rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.02em; color: var(--text-secondary);">Parts Alert</h4>
                    ${partsLowStock.length === 0 ? '<p class="text-muted" style="font-size: 0.82rem; margin: 0 0 1.25rem 0; display:flex; align-items:center; gap:4px;"><i class="fa-solid fa-circle-check" style="color:var(--status-success);"></i> All parts levels healthy.</p>' : `
                    <div style="display:flex; flex-direction:column; gap:0.5rem; margin-bottom:1.25rem;">
                        ${partsLowStock.map(item => `
                            <div style="display:flex; align-items:center; justify-content:space-between; background:var(--bg-hover); border:1px solid var(--border-color); border-radius:8px; padding:0.45rem 0.75rem;">
                                <div style="display:flex; flex-direction:column;">
                                    <span style="font-size:0.82rem; font-weight:700; color:var(--text-primary);">${escapeHtml(item.model)}</span>
                                    <span style="font-size:0.72rem; color:var(--text-secondary);">${escapeHtml(item.type || '-')}</span>
...
async function openStockAnalyticsModal() {
    if (!stockDataLoaded) await ensureStockDataLoaded();
    showModal('modalStockAnalytics');
    renderStockAnalytics('stockAnalyticsModalBody');
}
...
function setupStockEventListeners() {
    ...
    document.addEventListener('click', e => {
        const btn = e.target.closest('.admin-record-tab');
        if (!btn) return;
        currentAdminRecordFilter = btn.dataset.adminRecordFilter || 'all';
        document.querySelectorAll('.admin-record-tab').forEach(tab => {
