...
Max Shipping`,
    employees: ["Bryan", "Maksym", "Stacey", "Emily", "Bryson", "Thomas", "Jacque", "Amanda", "Darren", "Alex", "David"],
    pickupEmployees: ["Alex", "Bryan", "Max", "Valerii"],
    trailerEmailRecipients: 'purchasing@maxshipping.com',
    trailerEmailSubject: '[CLOPAY ARRIVED] Trip {trip_number} - Stop {stop_number}',
    trailerEmailBody: `Clopay trailer has arrived.
...
function isReceivingOrder(order) {
    return !isOutboundOrder(order) && !isPickupOrder(order) && !isTrailerOrder(order);
}
...
function updateDashboardChrome() {
    ...
    const isReceiving = !isOutbound && !isPickup && !isTrailers;
    ...
    document.getElementById('statusFilterWrapper')?.classList.toggle('hidden', !isReceiving);
    document.getElementById('carrierFilterWrapper')?.classList.toggle('hidden', isPickup || isTrailers);
    
    const tableTitle = document.getElementById('shipmentTableTitle');
    if (tableTitle) tableTitle.textContent = isTrailers ? 'Clopay Trailer Logs' : (isPickup ? 'Customer Pick Up Logs' : (isOutbound ? 'Shipped Out Logs' : 'Receiving Logs'));
    ...
    if (emptyText) {
        emptyText.textContent = isTrailers
            ? 'No Clopay trailer records yet or matching search criteria.'
            : isPickup
    ...
    if (tableHead) {
        tableHead.innerHTML = isTrailers ? `
            <tr>
                <th class="sortable" data-sort="bol_number">BOL #</th>
                <th class="sortable" data-sort="trip_number">Trip #</th>
                <th class="sortable" data-sort="stop_number">Stop #</th>
                <th>Trailer / Weight</th>
                <th class="sortable" data-sort="status">Status</th>
                <th class="sortable" data-sort="arrived_date">Timeline</th>
                <th>Documents / Issue</th>
                <th>Actions</th>
            </tr>
        ` : isPickup ? `
            <tr>
                <th class="sortable" data-sort="job_number">Job Number</th>
                <th class="sortable" data-sort="customer_name">Customer</th>
                <th class="sortable" data-sort="handled_by">Handled By</th>
                <th>Comments</th>
                <th class="sortable" data-sort="pickup_date">Pick Up Date</th>
                <th>Photos</th>
                <th>Actions</th>
            </tr>
        ` : isOutbound ? `
            <tr>
                ...
                <th class="sortable" data-sort="payment_type">Payment Type</th>
                <th class="sortable" data-sort="shipped_date">Shipped Date</th>
                <th>Photos</th>
                <th>Actions</th>
            </tr>
        ` : `
            <tr>
                ...
                <th class="sortable" data-sort="ordered_by">Ordered By</th>
                <th class="sortable" data-sort="ordered_date">Ordered Date</th>
...
function setDashboardTab(tab) {
    ...
    currentSortColumn = tab === 'outbound' ? 'shipped_date' : (tab === 'pickup' ? 'pickup_date' : 'id');
...
function renderDashboard() {
    ...
    const filteredOrders = orders.filter(order => {
        ...
        // Carrier filter
        if (!isPickupTab && !isTrailersTab && filterCarrier !== 'all') {
    ...
    if (currentSortColumn) {
        filteredOrders.sort((a, b) => {
            ...
            if (currentSortColumn === 'id') {
                valA = Number(valA) || 0;
                valB = Number(valB) || 0;
            } else if (currentSortColumn === 'ordered_date' || currentSortColumn === 'received_date' || currentSortColumn === 'shipped_date' || currentSortColumn === 'pickup_date') {
    ...
    if (filteredOrders.length === 0) {
        ...
        document.getElementById('recordCount').textContent = isTrailersTab ? 'Showing 0 trailers' : (isPickupTab ? 'Showing 0 pick up records' : (isOutboundTab ? 'Showing 0 shipped out records' : 'Showing 0 orders'));
        return;
    } else {
    ...
    filteredOrders.forEach(order => {
        ...
        if (isTrailersTab) {
            ...
            const statusClass = order.status === 'Arrived' ? 'received' : (order.status === 'Dispatched' ? 'in-transit' : 'ordered');
            const documentButton = documents.length ? `<button class="btn-scan attached" onclick="viewTrailerDocuments('${order.id}')"><i class="fa-solid fa-paperclip"></i> Docs (${documents.length})</button>` : '<span class="text-muted">No docs</span>';
            const issueButton = order.has_issue ? `<button class="btn-scan" onclick="viewTrailerIssuePhotos('${order.id}')" style="color:var(--status-error);background:var(--status-error-bg);"><i class="fa-solid fa-triangle-exclamation"></i> Issue (${issuePhotos.length})</button>` : '';
            const emailButton = order.arrived_date ? `<button class="btn-table-action receive" onclick="emailTrailerArrival('${order.id}')" title="Prepare Arrival Email"><i class="fa-solid fa-envelope"></i> Email</button>` : '';
            ...
            tr.innerHTML = `
                ...
                <td>${escapeHtml(order.stop_number)}</td>
                <td><strong>${escapeHtml(order.trailer_number || '-')}</strong><div class="received-info">${order.weight ? `${escapeHtml(order.weight)} lb` : 'No weight'}</div></td>
                <td><span class="badge ${statusClass}">${escapeHtml(order.status)}</span></td>
                <td><div class="received-info">Arrived: ${formatDateTimeDisplay(order.arrived_date)}<br>Dispatched: ${formatDateTimeDisplay(order.dispatched_date)}</div></td>
                <td><div class="attachment-actions">${documentButton}${issueButton}</div></td>
                <td><div class="action-buttons">${emailButton}<button class="btn-table-action" onclick="openTrailerModal('${order.id}')" title="Edit Trailer"><i class="fa-regular fa-pen-to-square"></i></button><button class="btn-table-action" onclick="generateOrderPdfReport('${order.id}')" title="Download PDF Report"><i class="fa-regular fa-file-pdf"></i></button></div></td>
            `;
        ...
        if (isPickupTab) {
            ...
            const photoCell = photoCount > 0 ? `
                <button class="btn-scan attached" onclick="viewPickupPhotos('${order.id}')" title="View Customer Pick Up Photos">
                    <i class="fa-solid fa-camera"></i> Photos (${photoCount})
                </button>` : `<span class="text-muted"><i class="fa-solid fa-minus"></i> None</span>`;
            ...
            tr.innerHTML = `
                ...
                <td>
                    <div class="action-buttons">
                        <button class="btn-table-action" onclick="openPickupModal('${order.id}')" title="Edit Customer Pick Up">
                            <i class="fa-regular fa-pen-to-square"></i>
                        </button>
                        <button class="btn-table-action" onclick="generateOrderPdfReport('${order.id}')" title="Download PDF Report">
        ...
        if (isOutboundTab) {
            ...
            const photoCell = photoCount > 0 ? `
                <button class="btn-scan attached" onclick="viewOutboundPhotos('${order.id}')" title="View Shipped Freight Photos">
                    <i class="fa-solid fa-camera"></i> Photos (${photoCount})
                </button>` : `<span class="text-muted"><i class="fa-solid fa-minus"></i> None</span>`;
            ...
            tr.innerHTML = `
                ...
                <td class="desc-col" title="${escapeHtml(order.ship_to_address || '')}">${escapeHtml(order.ship_to_address || '-')}</td>
                <td><span class="badge in-transit">${escapeHtml(order.payment_type || '-')}</span></td>
                <td>${formatDateTimeDisplay(order.shipped_date)}</td>
                <td>${photoCell}</td>
                <td>
                    <div class="action-buttons">
                        <button class="btn-table-action" onclick="openOutboundModal('${order.id}')" title="Edit Shipped Out">
                            <i class="fa-regular fa-pen-to-square"></i>
                        </button>
                        <button class="btn-table-action" onclick="generateOrderPdfReport('${order.id}')" title="Download PDF Report">
                            <i class="fa-regular fa-file-pdf"></i>
...
function openOutboundModal(orderId = null) {
    ...
    if (outbound) {
        ...
        if (outbound.shipped_photos && outbound.shipped_photos.length > 0) {
            ...
            document.getElementById('outboundPhotoDropZone').classList.add('hidden');
            document.getElementById('outboundPhotoSelectionStatus').classList.remove('hidden');
        }
    }
    
    document.getElementById('modalOutbound').classList.remove('hidden');
}
...
function openPickupModal(orderId = null) {
    ...
    if (pickup) {
        ...
        if (pickup.pickup_photos && pickup.pickup_photos.length > 0) {
            ...
            document.getElementById('pickupPhotoDropZone').classList.add('hidden');
            document.getElementById('pickupPhotoSelectionStatus').classList.remove('hidden');
        }
    }

    document.getElementById('modalPickup').classList.remove('hidden');
}
...
function openTrailerModal(orderId = null) {
    ...
    document.getElementById('modalTrailerTitle').textContent = trailer ? 'Edit Clopay Trailer' : 'Add Clopay Trailer';
    ...
    if (trailer) {
        ...
        if (trailer.documents?.length) {
            document.getElementById('trailerDocsDropZone').classList.add('hidden');
        ...
        if (trailer.issue_photos?.length) {
            document.getElementById('trailerIssuePhotoDropZone').classList.add('hidden');
...
async function handleOutboundSubmit(e) {
    ...
    const id = document.getElementById('outboundId').value || Date.now();
    ...
    let outbound = orders.find(o => o.id == id && isOutboundOrder(o)) || {
        id: Number(id),
...
async function handlePickupSubmit(e) {
    ...
    const id = document.getElementById('pickupId').value || Date.now();
    ...
    let pickup = orders.find(o => o.id == id && isPickupOrder(o)) || {
        id: Number(id),
...
function getPdfReportData(order) {
    ...
    const podFiles = order.invoice_filenames || (order.invoice_filename ? [order.invoice_filename] : []);
    const photos = [];
    if (isImageFilename(order.packing_slip_filename)) photos.push({ folder: 'scans', filename: order.packing_slip_filename, label: 'Packing Slip Image' });
    ...
    return {
        ...
        attachments: [order.packing_slip_filename, ...podFiles].filter(Boolean),
        photos
    };
}
...
async function generateOrderPdfReport(orderId) {
    ...
    addHeader(`${report.reference} | Generated ${formatDateTimeDisplay(getLocalDateTimeString())}`);
    report.fields.forEach(([label, value]) => {
        const text = value === undefined || value === null || value === '' ? '-' : String(value).replace('T', ' ');
    ...
    if (report.attachments.length) {
        ensureSpace(16 + report.attachments.length * 5);
        y += 5;
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
        doc.setTextColor(207, 46, 46);
    ...
    for (const photo of report.photos) {
        try {
            const image = await storageImageToJpeg(photo.folder, photo.filename);
            doc.addPage();
            addHeader(report.reference);
            doc.setFont('helvetica', 'bold');
            doc.setFontSize(13);
            ...
            const maxHeight = pageHeight - y - 16;
            const ratio = Math.min(maxWidth / image.width, maxHeight / image.height);
            doc.addImage(image.dataUrl, 'JPEG', (pageWidth - image.width * ratio) / 2, y, image.width * ratio, image.height * ratio);
        } catch (err) {
        ...
        content.innerHTML = `
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.25rem; font-size: 0.95rem; line-height: 1.5;">
                ...
                <div>
                    ...
                    <p><strong>Status:</strong> <span class="badge ${order.status === 'Arrived' ? 'received' : (order.status === 'Dispatched' ? 'in-transit' : 'ordered')}">${escapeHtml(order.status || 'Expected')}</span></p>
...
function setupUploads() {
    ...
    setupMultipleUpload(
        ...
        (files) => { selectedTrailerIssuePhotos = files; },
        true
    );
}
...
function populateEmployeeDropdowns() {
    ...
    const pickupList = settings.pickupEmployees || DEFAULT_SETTINGS.pickupEmployees;
...
function openAdminSettingsModal() {
    ...
    document.getElementById('adminPickupEmployees').value = (settings.pickupEmployees || DEFAULT_SETTINGS.pickupEmployees).join('\n');
...
async function handleAdminSettingsSubmit(e) {
    ...
    settings.pickupEmployees = pickupEmployeesArray;
...
function renderAdminShipmentsList() {
    ...
    filtered.forEach(order => {
        ...
        const reference = trailer ? order.trip_number : ((outbound || pickup) ? (order.job_number || '-') : order.po_number);
        const party = trailer ? `BOL ${order.bol_number}` : (pickup ? (order.customer_name || '-') : (outbound ? order.customer : order.supplier));
        const status = trailer ? order.status : (pickup ? 'Picked Up' : (outbound ? 'Shipped Out' : order.status));
        const statusClass = (outbound || pickup || trailer) ? 'in-transit' : order.status.toLowerCase().replace(' ', '-');
        const typeLabel = trailer ? 'TRAILER' : (pickup ? 'PICKUP' : (outbound ? 'OUT' : 'IN'));
...
async function adminDeleteOrder(orderId) {
    ...
    const pickup = isPickupOrder(order);
    const trailer = isTrailerOrder(order);
    let confirmMsg = trailer
        ? `ADMIN DELETE ACTION: Permanently delete Clopay Trailer Trip ${order.trip_number}?`
        : pickup
    ...
    if (!outbound && !pickup && !trailer && order.status === 'Received') {
