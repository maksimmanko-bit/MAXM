...
function setAuthenticatedUi(isAuthenticated) {
    ...
    ['btnDashboard', 'btnGlobalSearch', 'btnTaskCenter', 'btnHelp', 'btnAdmin', 'btnLogout', 'btnMobileHeaderMore'].forEach(id => {
...
function getRecordSearchValues(order) {
    ...
    if (isOutboundOrder(order)) return [order.job_number, order.customer, order.carrier, order.tracking_number, order.ship_to_address, order.payment_type, order.notes];
...
let stockRails = [];
let stockExtras = [];
let currentStockSubTab = 'doors'; // 'doors' | 'operators' | 'analytics'
...
function renderStockLoadingState() {
    ...
    document.getElementById('stockTableTitle').textContent = currentStockSubTab === 'doors' ? 'Doors Inventory' : currentStockSubTab === 'operators' ? 'Operators Inventory' : 'Stock Remaining Analytics';
...
function renderStockTable() {
    ...
    const title = isDoors ? 'Doors Inventory' : 'Operators Inventory';
    ...
    let items = isDoors ? sortStockDoors(stockDoors) : sortStockOperators(stockOperators);

    // Search filter (generic for both doors and operators)
    if (searchVal) {
        items = items.filter(item => {
            const base = [item.model, item.description, item.size || item.type].some(v => v && String(v).toLowerCase().includes(searchVal));
            if (base) return true;
    ...
    if (items.length === 0) {
        ...
        countEl.textContent = isDoors ? '0 items' : '';
        return;
    }

    emptyState.classList.add('hidden');
    document.querySelector('#stockRemainingSection .table-responsive')?.classList.remove('hidden');
    countEl.textContent = isDoors ? `${items.length} item${items.length !== 1 ? 's' : ''}` : '';
...
function openStockDetail(type, itemId) {
    const items = type === 'doors' ? stockDoors : (type === 'operators' ? stockOperators : (type === 'rails' ? stockRails : stockExtras));
    ...
    document.getElementById('stockDetailTitle').innerHTML = `
        <div style="display:flex; align-items:center; gap:0.75rem;">
            ...
            <button type="button" class="btn btn-secondary btn-icon" onclick="configureLowStockThreshold('${type}', '${escapeHtml(item.id)}')" title="Configure Low Stock Threshold" style="position: relative; width: 30px; height: 30px; border-radius: 8px; border: 1px solid var(--border-color); background: var(--bg-hover); cursor: pointer; display: inline-flex; align-items: center; justify-content: center; transition: all 0.2s ease; padding: 0; margin-left: 0.15rem;">
                <i class="fa-solid fa-gear" style="font-size: 1.05rem; color: var(--text-secondary);"></i>
                <span style="position: absolute; top: -5px; right: -5px; background: var(--brand-red); color: #fff; width: 14px; height: 14px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; font-weight: 900; box-shadow: 0 0 0 2px var(--bg-card); line-height: 1;" title="Low Stock Threshold Alerts">!</span>
            </button>
        </div>
    `;
    ...
    let infoHTML = `<div class="record-info-grid stock-detail-grid">`;
    if (type === 'doors') {
        infoHTML += `
    ...
    // Notes section (available for Doors and Operators only)
    const notesHTML = type !== 'rails' ? buildStockNotesHTML(item) : '';

    body.innerHTML = infoHTML + qtyHTML + notesHTML;
...
async function saveStockQty(type, itemId) {
    ...
    const newOnOrder = Math.max(0, parseInt(onOrderInput?.value) || 0);
    const newReOrder = Math.max(0, parseInt(reOrderInput?.value) || 0);
    ...
    try {
        ...
        let successMsg = `In Stock: ${newQty} | On Order: ${newOnOrder} | Re Order: ${newReOrder}`;
        if (doSync && diff !== 0) {
            if (targetRail) {
...
function buildStockNotesHTML(item) {
    ...
    const notesList = notes.map(note => {
        ...
        const doorInfo = note.door_id ? stockDoors.find(d => d.id === note.door_id) : null;
        const doorBadge = doorInfo ? `<span class="stock-note-rail-badge" style="background:rgba(59, 130, 246, 0.1); border-color:rgba(59, 130, 246, 0.25); color:var(--accent-blue);"><i class="fa-solid fa-door-open"></i> + Door ${doorInfo.model} ${doorInfo.size ? `(${doorInfo.size})` : ''} (${note.door_qty || note.qty})</span>` : '';
    ...
    if (hasRail) {
        railSelectHTML = `
            <div class="select-wrapper" style="flex:0 0 110px; height:38px;">
                <select id="stockNoteRailSize_${escapeHtml(item.id)}" title="Select Rail Size to sell with this operator" style="height:100%;">
                    <option value="7">Rail 7'</option>
                    <option value="8">Rail 8'</option>
                    <option value="10">Rail 10'</option>
                    <option value="None">No Rail</option>
    ...
    if (hasExtra) {
        extraSelectsHTML = item.extra_types.map(extraType => {
            ...
            return `
                <div class="select-wrapper" style="flex:0 0 140px; height:38px;">
                    <select class="stock-note-extra-select" data-extra-type="${escapeHtml(extraType)}" title="Select ${escapeHtml(extraType)} size/option to sell with this operator" style="height:100%;">
                        ${opts.map(o => {
                            const isDefaultSprocket = (extraType === 'Sprocket' && o.value === 'extra_sprocket_50B32');
                            return `<option value="${escapeHtml(o.value)}" ${isDefaultSprocket ? 'selected' : ''}>${escapeHtml(o.text)}</option>`;
                        }).join('')}
    ...
    return `
        <section class="record-detail-section" style="margin-top:1.25rem;">
            ...
            <div class="stock-note-form" id="stockNoteForm_${escapeHtml(item.id)}" style="margin-bottom: 1.25rem;">
                <div class="stock-note-form-row" style="display:flex; align-items:center; flex-wrap:wrap; gap:0.5rem;">
                    <div class="select-wrapper" style="flex:0 0 110px; height:38px;">
                        <select id="stockNoteType_${escapeHtml(item.id)}" style="height:100%;">
                    ...
                    ${railSelectHTML}
                    ${extraSelectsHTML}
                    <input type="text" id="stockNoteText_${escapeHtml(item.id)}" placeholder="Note / sales details..." style="flex:1; min-width:180px; height:38px; border:1px solid var(--border-color); background:var(--bg-hover); color:var(--text-primary); padding:0.5rem 0.75rem; border-radius:8px; font-size:0.85rem; outline:none;">
                    <input type="number" id="stockNoteQty_${escapeHtml(item.id)}" class="stock-note-qty-input" placeholder="QTY" min="1" value="1" title="QTY sold/reserved" style="width:70px; height:38px; border:1px solid var(--border-color); background:var(--bg-hover); color:var(--text-primary); padding:0.5rem 0.5rem; border-radius:8px; font-size:0.85rem; text-align:center; outline:none;">
                    <button type="button" class="btn btn-accent" onclick="addStockNote('${escapeHtml(item.id)}')" style="height:38px; border-radius:8px; padding:0 1.25rem; font-weight:700; display:inline-flex; align-items:center; justify-content:center; gap:0.25rem;"><i class="fa-solid fa-plus"></i> Add</button>
                </div>
                ${!isOp ? `
                <div style="display: flex; flex-direction: column; gap: 0.5rem; width: 100%; margin-top: 0.5rem;">
                    <label style="display: inline-flex; align-items: center; gap: 0.5rem; font-size: 0.78rem; font-weight: 700; cursor: pointer; color: var(--text-primary); margin: 0; background: var(--bg-hover); padding: 0.35rem 0.60rem; border-radius: 6px; border: 1px solid var(--border-color); width: max-content;">
                        <input type="checkbox" id="doorHasOperator_${escapeHtml(item.id)}" onchange="toggleDoorOperatorFields('${escapeHtml(item.id)}')" style="cursor: pointer;">
                        Is there Operator?
                    </label>
                    <div id="doorOperatorFields_${escapeHtml(item.id)}" class="hidden" style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center;">
                        <!-- Residential Operators Select -->
                        <div class="select-wrapper" style="width: 180px; height: 38px;">
                            <select id="doorOperatorSelect_${escapeHtml(item.id)}" onchange="handleDoorOperatorChange('${escapeHtml(item.id)}')" style="height:100%;">
                        ...
                        <!-- Rail Size Select -->
                        <div id="doorOperatorRailWrapper_${escapeHtml(item.id)}" class="select-wrapper hidden" style="width: 110px; height: 38px;">
                            <select id="doorOperatorRailSize_${escapeHtml(item.id)}" data-default-size="${defaultRailSize}" style="height:100%;">
                ...
                ` : ''}
                <p class="stock-note-hint" style="margin-bottom:0; margin-top:0.5rem;"><i class="fa-solid fa-circle-info"></i> ${hintText}</p>
            </div>
...
async function addStockNote(itemId) {
    ...
    try {
        ...
        if (container) {
            const updatedItem = (type === 'doors' ? stockDoors : (type === 'operators' ? stockOperators : stockRails)).find(i => i.id === itemId);
            if (updatedItem) {
...
async function deleteStockNote(itemId, noteId) {
    ...
    if (noteToDelete.linked_note_id) {
        if (type === 'doors' && noteToDelete.op_id) {
            ...
            linkedItem = stockDoors.find(d => d.id === noteToDelete.door_id);
            linkedItemType = 'doors';
        }
...
function renderStockAdminPanel(subType) {
    ...
    const items = isDoors ? sortStockDoors(stockDoors) : sortStockOperators(stockOperators);

    const addBtn = `<div style="margin-bottom:0.75rem; display:flex; justify-content:flex-end;">
        <button type="button" class="btn btn-accent" onclick="stockAdminAddItem()"><i class="fa-solid fa-plus"></i> Add ${isDoors ? 'Door' : 'Operator'}</button>
    </div>`;
...
function stockAdminAddItem() {
    const isDoors = stockAdminCurrentType === 'doors';
    const newId = `${isDoors ? 'door' : 'op'}_${Date.now()}`;
    if (isDoors) {
...
async function stockAdminSaveChanges() {
    ...
    const isDoors = stockAdminCurrentType === 'doors';
    const items = isDoors ? stockDoors : stockOperators;
            ...
            <section class="record-detail-section" style="margin-top:0;">
                ...
                <div class="rail-pills" style="flex-wrap:wrap; gap:0.5rem;">
                    ${items.map(item => {
                        ...
                        return `
                            <button type="button" class="rail-pill" onclick="closeAllModals(); openStockDetail('extras', '${escapeHtml(item.id)}')" style="font-size:0.85rem; padding:0.35rem 0.5rem 0.35rem 0.8rem;">
